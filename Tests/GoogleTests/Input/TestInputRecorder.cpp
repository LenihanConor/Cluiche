...
TEST_F(InputRecorderTest, RecordMultipleEvents)
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		TimeAbsolute timestamp(static_cast<float>(i) * 0.016f);  // 60 FPS
		recorder->RecordEvents(events, timestamp);
	}
...
TEST_F(InputRecorderTest, SaveAndLoadMultipleEvents)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		Event evt;
		evt.type = Event::EType::kKeyPressed;
		evt.key = static_cast<int>(EKey::W) + i;  // W, X, Y keys
		events.Add(evt);

		TimeAbsolute timestamp(static_cast<float>(i) * 0.1f);
		recorder->RecordEvents(events, timestamp);
	}
...
TEST_F(InputRecorderTest, UpdatePlaybackReturnsEvents)
{
	...
	TimeAbsolute recordTimestamp(1.0f);
	...
	TimeAbsolute playbackTimestamp(1.0f);
...
TEST_F(InputRecorderTest, UpdatePlaybackRespectsTimestamps)
{
	...
	evt1.key = static_cast<int>(EKey::Space);
	events1.Add(evt1);
	recorder->RecordEvents(events1, TimeAbsolute(1.0f));

	// Event at t=2.0
	EventData events2;
	Event evt2;
	evt2.type = Event::EType::kKeyPressed;
	evt2.key = static_cast<int>(EKey::Enter);
	events2.Add(evt2);
	recorder->RecordEvents(events2, TimeAbsolute(2.0f));
	...
	recorder->UpdatePlayback(TimeAbsolute(0.5f), playbackEvents1);
	EXPECT_EQ(playbackEvents1.Size(), 0u);

	// At t=1.0, should have first event
	EventData playbackEvents2;
	recorder->UpdatePlayback(TimeAbsolute(1.0f), playbackEvents2);
...
TEST_F(InputRecorderTest, RoundTripSingleEvent)
{
	...
	recordedEvt.key = static_cast<int>(EKey::Space);
	...
	EventData playbackEvents;
	playbackRecorder.UpdatePlayback(TimeAbsolute(1.0f), playbackEvents);

	// Verify event
	ASSERT_GT(playbackEvents.Size(), 0u);
	EXPECT_EQ(playbackEvents[0].type, Event::EType::kKeyPressed);
	EXPECT_EQ(playbackEvents[0].key, static_cast<int>(EKey::Space));
}
...
TEST_F(InputRecorderTest, RoundTripMultipleEvents)
{
	...
	evt1.key = static_cast<int>(EKey::Space);
	events1.Add(evt1);
	recorder->RecordEvents(events1, TimeAbsolute(0.0f));

	// Frame 2: Mouse button
	EventData events2;
	Event evt2;
	evt2.type = Event::EType::kMouseButtonPressed;
	evt2.mouseButton = static_cast<int>(EMouseButton::Left);
	events2.Add(evt2);
	recorder->RecordEvents(events2, TimeAbsolute(0.016f));
	...
	events3.Add(evt3);
	recorder->RecordEvents(events3, TimeAbsolute(0.032f));
	...
	EventData playback1;
	playbackRecorder.UpdatePlayback(TimeAbsolute(0.0f), playback1);
	ASSERT_GT(playback1.Size(), 0u);
	EXPECT_EQ(playback1[0].type, Event::EType::kKeyPressed);

	EventData playback2;
	playbackRecorder.UpdatePlayback(TimeAbsolute(0.016f), playback2);
	ASSERT_GT(playback2.Size(), 0u);
	EXPECT_EQ(playback2[0].type, Event::EType::kMouseButtonPressed);

	EventData playback3;
	playbackRecorder.UpdatePlayback(TimeAbsolute(0.032f), playback3);
...
TEST_F(InputRecorderTest, PlaybackWithoutLoadingFails)
{
	...
	EventData events;
	recorder->UpdatePlayback(TimeAbsolute(1.0f), events);
...
TEST_F(InputRecorderTest, StartRecordingStopsPlayback)
{
	...
	evt.key = static_cast<int>(EKey::Space);
	events.Add(evt);
	recorder->RecordEvents(events, TimeAbsolute(1.0f));
	recorder->StopRecording();
...
TEST_F(InputRecorderTest, RecordEmptyEventData)
{
	...
	recorder->RecordEvents(emptyEvents, TimeAbsolute(1.0f));
...
TEST_F(InputRecorderTest, RecordMultipleEventsInSingleFrame)
{
	...
	Event evt1;
	evt1.type = Event::EType::kKeyPressed;
	evt1.key = static_cast<int>(EKey::W);
	events.Add(evt1);

	Event evt2;
	evt2.type = Event::EType::kKeyPressed;
	evt2.key = static_cast<int>(EKey::A);
	events.Add(evt2);

	Event evt3;
	evt3.type = Event::EType::kMouseButtonPressed;
	evt3.mouseButton = static_cast<int>(EMouseButton::Left);
	events.Add(evt3);
...
TEST_F(InputRecorderTest, PlaybackMultipleTimesFromSameFile)
{
	...
	EventData events1;
	playback1.UpdatePlayback(TimeAbsolute(1.0f), events1);
	...
	EventData events2;
	playback2.UpdatePlayback(TimeAbsolute(1.0f), events2);
