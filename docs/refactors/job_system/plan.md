...
| 4 | Commit P1. Update plan rows 1–3 to Done. | `git status` clean | In Progress | haiku | Single commit. Plan update + code in same commit. |
| 5 | `JobSystemModule` holds `Dia::Core::JobSystem` by value (or `unique_ptr`). `DoStart` calls `instance.Initialize(0)`. `DoStop` calls `instance.Shutdown()`. Expose accessor (`JobSystem& GetJobSystem()`) so callers can reach the instance. Update `docs/specs/features/cluichetest/async-asset-loading/jobsystem-module.md` architecture section. | `dia run cluichetest` launches, exits clean | Pending | sonnet | If DiaApplicationFlow has no service-locator yet (audit assumption), accessor on the module is the fallback. |
| 6 | Commit P2. Update plan row 5 to Done. | `git status` clean | Pending | haiku | |
