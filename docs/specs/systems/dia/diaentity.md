...
diaentitytemplate is the gameplay-level entity system for the Dia engine. It is a **layered ECS
...
diaentitytemplate replaces the existing `IComponent`/`IComponentObject`/`IComponentFactory`
infrastructure (PD-003 / AD-005), which is barely used and philosophically opposed to the
target architecture. The old infrastructure is removed before diaentitytemplate is built (Decision
8 from the research). PD-003 and AD-005 will need a follow-up Superseded amendment once
diaentitytemplate is in place; this spec drives that conversation.
...
**Dependency chain:**
`diaentitytemplate → DiaMailbox → DiaCore (StringCRC, HandlePool<T>, DynamicArrayC, DIA_ASSERT, DIA_LOG_WARNING, jsoncpp wrapper)`

diaentitytemplate has no dependency on DiaApplicationFlow. Application integration (the EntityModule
adapter that plugs a Domain into the SimPU stage lifecycle) lives in application code, not
in diaentitytemplate.

## Responsibilities

- Provide **`Domain`** — the entity container that owns entity identity, component storage by type, the per-realm mailbox, the entity router, query caches, the reflection registry binding, and the end-of-frame mutation queue. (Renamed from "World" per Decision 16; reads cleanly next to Stage/Module/PU.)
...
- Provide **`IComponent` (diaentitytemplate's, not the old DiaCore one)** — the abstract base for all components. Every concrete component declares config, hooks asset triggers, exposes a typed gameplay interface to its owning system, and may opt into `DoUpdate(dt)` per Decision 6.
...
- Provide **`ComponentRegistry`** — process-global registry of `ComponentTypeDesc` keyed by `StringCRC`. Components register at static init via `DIA_COMPONENT`. Domain consults the registry to look up type metadata at runtime.
...
- Provide **query system** — `Domain::Query<ComponentA, ComponentB...>()` returns an iterator over entities that have all listed components. Caches matching entity sets keyed by signature; invalidates at `EndOfFrame()` based on collected mutations. Dynamic (no archetype storage) and good enough for v1's <1000-entity scale.
...
- Ship as `Dia/diaentitytemplate/diaentitytemplate.vcxproj` static library registered in `Cluiche.sln`.
- Provide `dia.entity.architecture.module.md` YAML module documentation.

## Non-Responsibilities

- **Application lifecycle.** diaentitytemplate does not depend on DiaApplicationFlow. The `EntityModule` adapter that owns a Domain and plugs into a stage's lifecycle is an application-level concern. Lives in CluicheTest / future games, not in diaentitytemplate.
- **Stage lifecycle.** Domains are created and destroyed by their owner. Stage-boundary hard cuts are the dominant pattern but not enforced by diaentitytemplate.
- **Cross-realm anything.** Entity handles are realm-local. There is no cross-realm reference, migration, or shared state. Editor-preview / test-fixture realms are supported by leaving `Domain` constructible as a normal value type, but diaentitytemplate has no API for talking between realms (Decision 16 — explicit non-decision).
- **System-side data.** Physics bodies, render objects, skeletons, state machines all live in their respective systems. diaentitytemplate holds handles into those systems via component fields.
...
- **Animation graph / physics step / render extraction.** Those run in their owning systems. diaentitytemplate provides queries and component access.
...
- **General reflection for the engine.** Reflection lives **inside diaentitytemplate** and applies to component types only. If a second consumer (asset definitions, editor configs) needs reflection, it gets lifted to a `DiaReflection` module then — not preemptively. Per the research addendum's scope discipline.
- **General relationship graph.** Only parent/child hierarchy. Arbitrary relationship edges between entities are out of scope.
- **Old IComponent / IComponentObject / IComponentFactory.** Those are removed before diaentitytemplate is built (Decision 8). diaentitytemplate introduces a new `IComponent` in its own namespace.
...
namespace Dia::Entity {
    // Domain-local generational handle. Equivalent to Handle<EntityTag> from DiaCore.
    using Entity = Dia::Core::Handle<class EntityTag>;

    class Domain {
    public:
        Domain(); // capacity is template-parameterised on a typedef alias; see decisions
        ~Domain();
        ...
        // The entity router is auto-registered with the mailbox on Domain construction.
...
namespace Dia::Entity {
    ...
    public:
        ...
        virtual void OnAttach(Domain& realm, Entity self) {}

        // Called once before destruction. The component is still attached and queryable.
        virtual void OnDetach(Domain& realm, Entity self) {}
        ...
        virtual void DoUpdate(Domain& realm, Entity self, float dt) {}

        // Called when a referenced asset has finished loading. Fires per asset trigger.
        virtual void OnAssetLoaded(Domain& realm, Entity self, Dia::Core::StringCRC assetId) {}
...
namespace Dia::Entity {
    ...
    public:
        explicit EntityRouter(Domain& realm);
...
namespace Dia::Entity {
    ...
    namespace Hierarchy {
        ...
        void QueueSetParent(Domain& realm, Entity child, Entity parent);

        // Queue subtree destruction. Applied at EndOfFrame.
        void QueueDestroySubtree(Domain& realm, Entity root);
    }
}
```
...
Implementation: query results are cached per (component-type-set) signature inside Domain.
...
`Domain` implements `IEntityInspectable`. Tier (c) live structural edit is intentionally
...
|---------|-------------|------|--------|
| remove-old-icomponent | Delete `Dia/DiaCore/Architecture/Components/`, migrate `SkeletonComponent` and `StateMachineComponent` consumers, remove related tests. Per Decision 8. Must land before any diaentitytemplate code is written. | — | Done |
| foundation | `Domain`, `Entity`, `IComponent` base, component type-pools (one `HandlePool<T>` per component type), entity create/destroy lifecycle, basic `GetComponent`/`HasComponent`, end-of-frame structural change application. | [foundation.md](../../features/dia/diaentitytemplate/foundation.md) | Approved |
| reflection | `DIA_COMPONENT` macro, `FIELD` macro, `ComponentTypeDesc`, `ComponentRegistry`, JSON load/save thunks, schema version field. Per Decision 14. | [reflection.md](../../features/dia/diaentitytemplate/reflection.md) | Approved |
| blueprint-loader | `JsonBlueprintLoader` interface + concrete impl, versioned JSON schema with separate references block, asset-trigger callback into components. Per Decision 11. | [blueprint-loader.md](../../features/dia/diaentitytemplate/blueprint-loader.md) | Approved |
| component-deps-and-refs | `REQUIRES` macro with hard validation at attach, `EntityRef<TComponent>` typed reference slots, two-pass blueprint resolution (instantiate then patch references). Per Decisions 4, 7. | [component-deps-and-refs.md](../../features/dia/diaentitytemplate/component-deps-and-refs.md) | Approved |
| hierarchy | `ParentComponent`, `ChildBufferComponent`, `Hierarchy::QueueSetParent`/`QueueDestroySubtree`, cycle rejection in debug, destroy-subtree default cascade. Per Decision 15. | [hierarchy.md](../../features/dia/diaentitytemplate/hierarchy.md) | Approved |
| mailbox-router | `EntityRouter`, address encoding (`MakeEntityAddress` etc.), auto-registration of router with domain's mailbox, resolve implementations for all four kinds (Entity/All/ComponentType/Self). | [mailbox-router.md](../../features/dia/diaentitytemplate/mailbox-router.md) | Approved |
| query-system | `Domain::Query<...>`, signature-keyed query cache, end-of-frame invalidation/rebuild. Per Decision 10. | [query-system.md](../../features/dia/diaentitytemplate/query-system.md) | Approved |
| editor-inspection | `IEntityInspectable` implementation on Domain, reflection-driven `ReadField`/`WriteField`, mailbox log accessor. Tiers (a) + (b) per Decision 17. | [editor-inspection.md](../../features/dia/diaentitytemplate/editor-inspection.md) | Approved |
| update-loop | `Domain::Update(dt)` walks DoUpdate-opted components in registration order, then entity-index order. Per Decision 6. | [update-loop.md](../../features/dia/diaentitytemplate/update-loop.md) | Approved |
| module-and-build | `diaentitytemplate.vcxproj`, registration in `Cluiche.sln`, `dia.entity.architecture.module.md` YAML, dependency edge in `dia_modules.py`. | [module-and-build.md](../../features/dia/diaentitytemplate/module-and-build.md) | Approved |
...
**Explicitly excluded:**
- **DiaApplicationFlow** — Domain has no knowledge of stages, modules, or PUs. Application integration lives outside diaentitytemplate.
- **DiaSerializer** — Reflection-driven JSON load/save lives inside diaentitytemplate. Reusing `DiaSerializer::MetadataValue` was considered but rejected: components have typed schemas (rich field types incl. asset handles, entity refs), not free-form metadata bags. DiaSerializer remains the right tool for engine definition files; diaentitytemplate is the right tool for component schemas.
- **Any specific gameplay system** (DiaRigidBody2D, DiaRig2D, DiaStateMachine, DiaGraphics) — components in those systems consume diaentitytemplate (their components live in their respective modules, register via `DIA_COMPONENT`, use diaentitytemplate from below). diaentitytemplate does not depend on any of them.

**Dependents (once diaentitytemplate ships):**
- **CluicheTest application code** — `EntityModule` adapter that owns a Domain, plugs into DiaApplicationFlow v2 stage lifecycle (DoStart loads blueprints → returns kLoading until assets resolve → kReady; stage transition destroys realm)
...
- **Garbage collection of orphan handles into systems.** When a component is removed, its `OnDetach` is called and the component is responsible for releasing system-side handles. diaentitytemplate does not enforce or audit this.
...
|----|----------|-----------|-------|--------|---------|
| SD-ENT-001 | Container is `Domain`, not `Domain` or `World` | "World" and "Scene" overlap with stage/rendering concepts. "Domain" was the original pick but conflicts with `Dia::Core::Containers` namespace proximity. `Domain` is short, unambiguous, no namespace collisions. | All | Accepted | Yes |
| SD-ENT-002 | Systems own their data; components are typed adapters (config + asset trigger + interface) | Foundational architectural stance. Distinguishes diaentitytemplate from archetype-chunk ECS designs. Per research Decision 0 / explore.md. | All | Accepted | Yes |
...
| SD-ENT-005 | Reflection is component-only and lives inside diaentitytemplate (not a generic `DiaReflection` module) | Scope discipline. Lift to a separate module only if a second consumer emerges. Per research addendum. | Reflection | Accepted | Yes |
...
| SD-ENT-012 | Structural changes (create entity, destroy, add/remove component, set parent) are queued and applied at `Domain::EndOfFrame()` | Avoids iterator invalidation and order-dependent bugs during gameplay update. Per research Decision 10. | Mutation | Accepted | Yes |
...
| SD-ENT-015 | `Domain::Update(dt)` walks DoUpdate-opted components in component-type-registration order, then entity-index order | Predictable, simple, deterministic. No per-component scheduling. Per user choice during system spec interview. Decision 6 from research is fully realised here. | Update | Accepted | Yes |
...
| SD-ENT-017 | Domain is non-copyable, non-movable | Owns inline component pools and outstanding handles tied to its address. Movability would require remapping all handles. | All | Accepted | Yes |
...
| SD-ENT-020 | The old `IComponent` infrastructure is removed before diaentitytemplate is built | Clean slate. The two existing consumers (SkeletonComponent, StateMachineComponent) are migrated as part of removal — they become diaentitytemplate components in their own modules. Per research Decision 8. | Migration | Accepted | Yes |
| SD-ENT-021 | PD-003 / AD-005 require a Superseded amendment once diaentitytemplate ships | Those decisions reference the old `IComponent` model. They cannot remain Accepted as written. Amendment text is out of scope of this system spec but the requirement is captured here. | Platform / App decisions | Accepted | Yes |
...
| PD-002 | Platform | ProcessingUnit/Phase/Module architecture | diaentitytemplate does not own a PU/Module. Application code wraps Domain in an `EntityModule` that lives in SimPU. diaentitytemplate itself has no PU/Phase coupling. |
| PD-003 | Platform | Component-based entities (IComponent/IComponentObject) | **Conflict — Pending Supersede.** This decision references the old IComponent model that diaentitytemplate replaces. Per SD-ENT-021, PD-003 needs a Superseded amendment. diaentitytemplate introduces a new `Dia::Entity::IComponent` distinct from the old one, satisfying the spirit of "component-based entities" with a different concrete model. Open question: track in CONFLICT table below. |
...
| PD-005 | Platform | x64 only | `diaentitytemplate.vcxproj` targets x64 exclusively. |
| PD-006 | Platform | Visual Studio project files are source of truth | `diaentitytemplate.vcxproj` and `.vcxproj.filters` created and manually maintained. |
...
| PD-008 | Platform | Directory.Build.props owns OutDir/IntDir/toolchain | `diaentitytemplate.vcxproj` must NOT override OutDir, IntDir, PlatformToolset, WindowsTargetPlatformVersion, or LanguageStandard. |
| PD-009 | Platform | Generated output under `Cluiche/out/<AppName>/` | diaentitytemplate does not generate output. The blueprint loader reads JSON from asset paths but does not write generated artefacts. |
| PD-010 | Platform | `.diagame` is the project root file; `.diastage` declares stage metadata | diaentitytemplate does not directly couple to `.diagame` or `.diastage`. Application's `EntityModule` reads stage manifests and feeds blueprint asset paths to diaentitytemplate via the loader interface. |
...
| AD-004 | Dia App | ProcessingUnit/Phase/Module for app structure | diaentitytemplate does not own a PU. Composition with DiaApplicationFlow is the application's job. |
| AD-005 | Dia App | Component-based entities (IComponent/IComponentObject) | **Conflict — Pending Supersede.** Same as PD-003. AD-005 references the old model; diaentitytemplate replaces it. Open question below. |

## Conflicts and Open Questions

| ID | Issue | Resolution |
|----|-------|-----------|
| C-1 | PD-003 and AD-005 reference the old `IComponent`/`IComponentObject` model | **Resolved.** Both decisions amended to `Superseded by diaentitytemplate.md` in `docs/specs/platform/Cluiche.md` and `docs/specs/applications/dia.md` (2026-05-24, per SD-ENT-021). |

## AI Review Questions

| # | Section | Question | Answer |
|---|---------|----------|--------|
| 1 | Domain capacity | Domain needs a fixed entity capacity (HandlePool requires compile-time capacity). What's the v1 number? | The research locked <1000 entities per stage as the v1 scale. A `kMaxEntitiesPerDomain = 1024` default in `Domain.h` (template-parameterised on `Domain`'s own type so consumers can specialise) is the v1 target. Components type-pools size-default to `kMaxEntitiesPerDomain` as well — sparse pools are cheap because they're not allocated until a component type registers. |
| 2 | Component pools sized per-type | Each component type wants its own HandlePool capacity. How is this sized? | Default to `kMaxEntitiesPerDomain` for all component types (worst case: every entity has every component). Components that are known-rare (e.g. `BossEnemyTagComponent`) can override at registration via an optional second arg to `DIA_COMPONENT_REGISTER`. v1 doesn't optimise this; revisit when memory is actually a concern. |
...
| 8 | DoUpdate ordering and Mailbox.Drain | When does mailbox delivery happen relative to DoUpdate? | Frame timing per research summary: (1) frame start → process mailbox (drain last frame's queued messages, deliver to subscribers); (2) Domain::Update(dt) → walk DoUpdate components; (3) gameplay update outside Domain → drives queries, sends messages; (4) Domain::EndOfFrame() → apply structural mutations, rebuild query caches, queue mailbox messages for next-frame drain. Mailbox is delivered at frame start, not in EndOfFrame, to give DoUpdate a chance to react to inbound messages. |
...
| 10 | All address kind cost | Resolving "All" means iterating every subscriber for the type. Is this expensive? | At v1 scale (<1000 entities × <10 subscribers per type) it's fine. If a frame has 100 broadcasts, that's 100×10 = 1000 ops. The bigger risk is overflowing `SubscriberSet`'s 64-element capacity (which DiaMailbox sized as a placeholder, see DiaMailbox AI Q14). diaentitytemplate is the consumer that should resize SubscriberSet to `kMaxEntitiesPerDomain`. |
| 11 | Hierarchy destroy-cascade | If destroying the parent destroys the subtree, what about external EntityRefs into the subtree? | They become invalid (handle generation mismatches). Holders that read them get nullptr from `GetComponent<TComponent>()`. Components that need cleanup-on-target-destroy should subscribe to a `EntityDestroyedMessage` via the mailbox (a message diaentitytemplate emits during the destroy pass). Open question: should this message be in v1 or deferred? **v1.** It's needed for any non-trivial gameplay. |
...
| 13 | Mailbox lifetime per realm | Domain owns one Mailbox. What if a consumer wants their own mailbox for a non-entity concern? | They construct their own. DiaMailbox's caller-owned model means a consumer in CluicheTest can have a stage-scoped Mailbox separate from the entity mailbox. diaentitytemplate owns one for entity routing; nothing prevents others. |
...
| 15 | Old IComponent migration | The two existing consumers (SkeletonComponent, StateMachineComponent) — what happens to them during the remove-old-icomponent feature? | They are deleted. DiaRig2D and DiaStateMachine continue to own their respective objects (Skeleton, StateMachine) directly — the system-owned-data principle. New diaentitytemplate components (`Rig2DComponent`, `StateMachineComponent` in Dia::Entity:: but living in their respective module folders) are created in their original modules later, after diaentitytemplate ships, as needed. The remove feature deletes; subsequent features in those modules re-add the component layer on top of diaentitytemplate. |
| 16 | Test consumer for diaentitytemplate | How is diaentitytemplate exercised before any real gameplay system depends on it? | A `TestDomainComponent` and a small CluicheTest blueprint exercise the foundation, reflection, blueprint loading, references, hierarchy, queries, mailbox routing, and inspection. DummyStage is the natural host. Once that smoke test works, real component types (Rig2D, StateMachine, RigidBody2D) migrate over. |
| 17 | vcxproj placement | Where does `diaentitytemplate.vcxproj` live? | `Dia/diaentitytemplate/diaentitytemplate.vcxproj` — same pattern as every other Dia module. Added to `Cluiche.sln` under the `Dia` solution folder. |
