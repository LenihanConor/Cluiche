...
| 11 | Entity placement — Add: "+ Entity" button → blueprint picker (from asset catalogue) → new entity with empty instance_data | Add entity → appears in hierarchy with auto-ID | Done | sonnet | `SceneMutator::AddItem` generates `{blueprintId}_{N}` id, sets defaults (camera active=false unless first, light affects_layers=[]). `scene_editor.add_item` + `scene_editor.get_available_blueprints` handlers registered. Build: PASSED. |
