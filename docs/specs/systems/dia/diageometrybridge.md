...
|---------|-------------|------|--------|
| Projection Helpers (3D→2D) | `Project()` overloads for AABB, Sphere, Triangle, Frustum, Ray, Plane, Vector3D. Mandatory `Axis2D` parameter. | [projection-helpers.md](../../features/dia/diageometrybridge/projection-helpers.md) | Draft |
| Lift Helpers (2D→3D) | `Lift()`, `LiftToSphere()`, `LiftToAABB()` overloads for AARect, Circle, Triangle, Ray, Vector2D. Caller-supplied missing-axis extent. | [lift-helpers.md](../../features/dia/diageometrybridge/lift-helpers.md) | Draft |
