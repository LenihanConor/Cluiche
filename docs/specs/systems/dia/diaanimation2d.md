...
- **DiaApplication** — no dependency; callers invoke animation APIs from within their phases/modules
...
| 11 | Architecture | How does DiaAnimation2D interact with the ProcessingUnit/Phase execution model? | DiaAnimation2D has no DiaApplication dependency. `AnimationEvaluator::Evaluate(dt)` runs the internal pipeline in the correct order (FK → clip → spring → blend). Game code calls `Evaluate(dt)` from within their own phases, then runs DiaIK2D post-process and final FK. The raw APIs (SpringChain, AnimClipPlayer, PoseBlendStack) are still public for advanced manual orchestration. |
