...
| Baseline Commands | Registers `dia.app.quit` and `dia.app.report` with DiaAPI (JSON callback path); extends DiaAPI with dotted-name validation and JSON command dispatch; consistent `{success, data/error}` envelope | [baseline-commands.md](../../features/dia/diaapplicationflow/baseline-commands.md) | Approved |
