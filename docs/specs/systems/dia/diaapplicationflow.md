...
| Transition Guards | Generic veto-on-transition hook; modules register opaque `GuardResult Check()` callbacks; pull model checked each Update while pending; first consumer is RemoteControlModule (orchestrator hold), reusable for loading screens / asset gating | [transition-guards.md](../../features/dia/diaapplicationflow/transition-guards.md) | Done |
| Baseline Commands | Registers `dia.app.quit` and `dia.app.report` with DiaAPI (JSON callback path); extends DiaAPI with dotted-name validation and JSON command dispatch; consistent `{success, data/error}` envelope | [baseline-commands.md](../../features/dia/diaapplicationflow/baseline-commands.md) | Done |
...
| Manifest Heap Modules | Migrate `ProcessingUnitDeclaration.modules` from `DynamicArrayC<ModuleDeclaration, 32>` to heap-allocated `DynamicArray<ModuleDeclaration>` to eliminate ~60 KB stack bloat in `ApplicationManifestV3` | [manifest-heap-modules.md](../../features/dia/diaapplicationflow/manifest-heap-modules.md) | Deferred |
