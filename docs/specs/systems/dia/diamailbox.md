...
The immediate motivating consumer is diaentitytemplate (entity-to-entity, component-to-component,
...
choose.md). After review, those four kinds are correctly the responsibility of diaentitytemplate's
...
- **Knowledge of entities, components, physics bodies, or any domain concept.** Routers live in their respective domain modules (e.g. `EntityRouter` in diaentitytemplate). DiaMailbox is a transport primitive only.
...
namespace Dia::Mailbox {
    ...
    public:
        ...
        // Returns false if routerId is unregistered. Domain consumers (e.g. diaentitytemplate's
...
namespace Dia::Mailbox {
    ...
    // sent to that address. Routers live in domain modules (EntityRouter in diaentitytemplate, etc.).
...
but the chosen number gets revisited as soon as a real consumer (diaentitytemplate) sizes its
...
|---------|-------------|------|--------|
| address-and-types | `Address` value type, `SubscriberId`, `SubscriptionHandle`, `SubscriberSet` alias, `OverflowPolicy` enum, public types only | [address-and-types.md](../../features/dia/diamailbox/address-and-types.md) | Done |
| typed-queue | `Mailbox::RegisterType`, `Send`, `Drain` — per-type ring buffers with compile-time capacity and overflow policy | [typed-queue.md](../../features/dia/diamailbox/typed-queue.md) | Done |
| subscriptions | `Mailbox::Subscribe` / `Unsubscribe`, `SubscriptionHandle` lifetime tied to issuing Mailbox | [subscriptions.md](../../features/dia/diamailbox/subscriptions.md) | Done |
| routers | `IMailboxRouter` interface, `RegisterRouter`, `GetRouter`, `Resolve` typed wrapper; `MockRouter` + `MailboxFixture` test utilities | [routers.md](../../features/dia/diamailbox/routers.md) | Done |
| module-and-build | `DiaMailbox.vcxproj`, registration in `Cluiche.sln`, `dia.mailbox.architecture.module.md` YAML | [module-and-build.md](../../features/dia/diamailbox/module-and-build.md) | Done |
...
**Explicitly excluded:**
- **diaentitytemplate** — DiaMailbox is a foundation layer for diaentitytemplate, not the other way around. The entity router is owned by diaentitytemplate.
...
- **diaentitytemplate** — owns the entity router; entity-to-entity messaging routes through DiaMailbox
...
|----|--------|----------|----------------------------|
| PD-001 | Platform | StringCRC for all entity/component IDs | Router IDs are `StringCRC`; subscriber identity is opaque (`SubscriberId`) but diaentitytemplate will populate it from `Handle<Entity>` bits — DiaMailbox itself uses no raw strings. |
...
| 5 | Router pointer ownership | `RegisterRouter(IMailboxRouter*)` — Mailbox stores raw pointer. Who owns lifetime? | Caller. The router is typically owned by a domain module (e.g. diaentitytemplate owns its `EntityRouter` as a member). The module unregisters before destruction. DiaMailbox does not own routers — that would couple lifetimes incorrectly (e.g. mailbox might outlive the entity world). |
...
| 14 | SubscriberSet capacity | `using SubscriberSet = DynamicArrayC<SubscriberId, 64>` is a placeholder. When does this get sized properly? | Once diaentitytemplate is specced, the capacity is sized to `kMaxEntities` (or whatever diaentitytemplate caps a stage at) so a broadcast can fan out to every entity in the worst case. Updating this is a compile-time constant change with no API churn. |

## Status

`Done` — All 5 features implemented and verified (2026-05-21). 49/49 tests GREEN.
