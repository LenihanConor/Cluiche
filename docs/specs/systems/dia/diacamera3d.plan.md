...
| 4 | Implement `ICameraBehaviour3D.h` + `CameraBehaviourRegistry3D.h/.cpp` (self-registering factory, kMaxBehaviourTypes=16) | `TestCameraBehaviourRegistry3D.cpp` — register, create, unknown type returns nullptr | Not Started | sonnet | |
| 5 | Implement `CameraRegistry3D.h/.cpp` (Register, Unregister, Has, AttachBehaviour, DetachBehaviour, SetActive, GetActive/Id, Get const+mut, UpdateAll) | `TestCameraRegistry3D.cpp` — CRUD, active camera, behaviour dispatch order | Not Started | sonnet | |
