...
| 11 | Observability: add DIA_LOG_* to all handler failure paths + DIA_TRACE_ZONE on BuildListJson / BuildPropertyJson / BuildAvailableComponentsJson | Log output visible on bad file path, malformed JSON, missing key | Not Started | haiku | Plugin.cpp handler failure returns currently log nothing; FileHandler, ListController, PropertyController silent |
| 12 | GoogleTests: BlueprintFileHandler (round-trip, bad path, malformed JSON, TopLevelKeyForExtension), BlueprintListController (IsBlueprintType, BuildListJson empty/mixed), BlueprintPropertyController (BuildPropertyJson, BuildAvailableComponentsJson, BuildUsageJson) | All test cases pass in `dia run googletest` | Not Started | sonnet | Pure-logic classes with no PU/module deps — straightforward to test without mocks |
