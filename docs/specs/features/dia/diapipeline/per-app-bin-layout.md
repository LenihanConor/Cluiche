...
2. Static and dynamic libraries land in a shared linker-visible pool (`Cluiche/bin/$(Configuration)/$(Platform)/`) — unchanged from today
...
1. `Directory.Build.props` branches `OutDir` on `$(ConfigurationType)`: `Application` → `Cluiche/bin/$(MSBuildProjectName)/$(Configuration)/$(Platform)/`; `StaticLibrary`/`DynamicLibrary` → `Cluiche/bin/$(Configuration)/$(Platform)/`
2. `path_resolver.py` `resolve_out_dir` accepts an optional `app_name` parameter; when provided it returns `Cluiche/bin/<app_name>/<config>/<platform>/`; when `None` it returns the existing shared pool path (backward compatible)
...
6. `GoogleTests.vcxproj` post-build event corrected from `$(OutDir)` to `$(TargetDir)` to match all other targets
7. `test_dia_pipeline.py` tests updated to cover per-app and shared-pool path resolution
8. A clean build produces three runnable folders: `bin/CluicheEditor/Debug/x64/`, `bin/CluicheTest/Debug/x64/`, `bin/GoogleTests/Debug/x64/`
...
- Adding a named `bin/lib/` subfolder for static libs — the shared pool is sufficient

## Tasks

| # | Task | Description |
|---|------|-------------|
| 1 | Update `Directory.Build.props` | Branch `OutDir` on `$(ConfigurationType)`; Application targets go to per-app subfolder |
| 2 | Update `path_resolver.py` | Add optional `app_name` to `resolve_out_dir` and `resolve_variables`; backward compatible |
| 3 | Update `package_stage.py` | Pass target name as `app_name` in both `run()` and `run_deploy()` |
| 4 | Fix `GoogleTests.vcxproj` | Change post-build `$(OutDir)` → `$(TargetDir)` |
| 5 | Update `test_dia_pipeline.py` | Update existing resolver tests; add per-app and shared-pool cases |
| 6 | Add subprocess deploy rule to `pipeline.toml` | Add `DiaUICEF_TestSubprocess.exe` copy rule to `cluicheeditor` deploy section so CEF can find it |
...
| PD-008 `Directory.Build.props` owns OutDir/IntDir | No per-project OutDir overrides | Compliant — OutDir change goes into `Directory.Build.props` only; zero per-project `.vcxproj` changes |
...
| SD-PIPE-001 `pipeline.toml` is single source of truth | All pipeline config in `pipeline.toml` | Compliant — deploy rules unchanged; `$(OutDir)` resolution now per-app but still driven by `pipeline.toml` |
| SD-PIPE-002 Fixed stage ordering | compile-code → build-assets → deploy | Compliant — stage order unchanged |
| SD-PIPE-003 `$(OutDir)` resolved from `Directory.Build.props` conventions at runtime | Path resolution consistent with MSBuild | Compliant — `path_resolver.py` mirrors the new `Directory.Build.props` convention |
| SD-PIPE-006 Deploy coexists with `.vcxproj` post-build events | No forced xcopy removal | Compliant — all post-build events already call DIA CLI; no xcopy present; coexistence is clean |
...
| 3 | Linker paths | After moving Application outputs to per-app subfolders, are there any `.vcxproj` files that hardcode `bin/Debug/x64` as an `AdditionalLibraryDirectories` path that would break? | No — grep confirms zero hardcoded `bin/Debug` or `bin/Release` paths across all `.vcxproj` files. All use `$(OutDir)` or `$(SolutionDir)` macros and will follow the new layout automatically. |
| 4 | Acceptance criteria 8 | `bin/GoogleTests/` — the MSBuild project is named `GoogleTests`; the pipeline target is `googletest` (lowercase). Does the per-app folder name follow the MSBuild project name (`GoogleTests`) or the pipeline target name (`googletest`)? | MSBuild project name — derived from the `.vcxproj` filename via option C. Folders are `bin/GoogleTests/`, `bin/CluicheEditor/`, `bin/CluicheTest/`. |
| 5 | `test_dia_pipeline.py` | Should the existing `test_resolve_out_dir_debug` and `test_resolve_out_dir_release` tests be updated in-place (breaking old behaviour) or kept alongside new per-app tests? | Existing tests kept and remain valid — `app_name=None` preserves the shared-pool path. New per-app test cases added alongside them. |
| 6 | `DiaUICEF_TestSubprocess.exe` | This exe is currently in the flat `bin/Debug/x64/`. Its `ConfigurationType` is `Application`. Which app folder should it land in — its own (`DiaUICEF_TestSubprocess`) or under `CluicheEditor` which depends on it? | `DiaUICEF_TestSubprocess` lands in its own `bin/DiaUICEF_TestSubprocess/Debug/x64/` by convention. A DIA CLI deploy rule in the `cluicheeditor` target copies it into `bin/CluicheEditor/Debug/x64/` alongside `CluicheEditor.exe` (required by CEF for subprocess discovery). No per-project `.vcxproj` exception needed. |
