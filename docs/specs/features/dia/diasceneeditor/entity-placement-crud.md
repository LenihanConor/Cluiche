...
Add, duplicate, delete, enable/disable, and rename entity instances in a `.diascene`. When adding a new entity, the user selects a blueprint (`.diaentity`); the new placement starts with empty `instance_data` (inheriting all defaults from the blueprint). Duplicate clones an existing entity with a position offset. Context menu provides all operations.
...
- Dropdown lists all registered `.diaentity` files from the DiaAssetCatalogue (per SED-SCN-010)
