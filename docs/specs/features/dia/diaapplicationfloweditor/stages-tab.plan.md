...
**Architecture invariant for this feature:** purely additive React change. The manifest already exposes `stages: StageV2[]` (ordered), `initialStage: string`, and `autoStages: string[]`. The live store already exposes `connectionState` and `activeStage`. No C++, no new bridge command (live click-to-transition reuses the existing `app.transitionTo` from `live-transition-trigger`).

**Edge model is auto-only:** an arrow from `stages[i]` to `stages[i+1]` is rendered iff `stages[i].name ∈ autoStages`. Manual stages have no outgoing edge. **Layout is linear** left-to-right in array order, no manual positioning, no persisted layout sidecar. **Graph is read-only** — all structural edits stay in the existing `StageConfiguration` sidebar (no duplicated edit surface).

**Live overlay:** active stage gets a pulsing green ring (1.6s period), and if the active stage is in `autoStages`, its outgoing edge animates with a flowing dashed pattern. Clicking any non-active stage in live mode dispatches `bridgeRequest('app.transitionTo', { stage: <name> })`. Clicking in static mode is ignored.
