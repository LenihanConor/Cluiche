...
| T-01 | PowerShell script: move `Win32WndProcChain.h/.cpp` → `Dia/DiaBgfx/Imgui/`; update DiaSFML.vcxproj, DiaBgfx.vcxproj, DiaBgfx.vcxproj.filters; rename namespace to `Dia::Bgfx`; update BgfxImGuiBackend.cpp include | Build passes, ImGui works in debug | Todo | sonnet | Prep commit — do before any DiaSFML source changes |
| T-02 | Remove `IWindow::SetActive(bool)` from IWindow interface; remove from Win32Window and DiaSFML::Window; remove stale comment in KernelModule | Build passes | Done | haiku | SetActive removed from IWindow, Win32Window, DiaSFML::Window; stale context deactivation comment remains (calls Canvas::SetActiveContext, not Window::SetActive) |
| T-03 | Grep dead EType entries; remove `kMouseWheelMoved`/`kMouseEntered`/`kMouseLeft` from Event.h if no consumers; add `ListenForInputSources(BitArray8)` to `IInputSource` | Build passes | Todo | haiku | AC-05; flag if consumers found |
...
| T-05 | Create `Dia/DiaSDL/` folder; scaffold `DiaSDL.vcxproj`, `DiaSDL.vcxproj.filters`, `dia.sdl.architecture.module.md`; add to `Cluiche.sln` | Project loads in VS | Todo | haiku | AC-11/AC-12 |
| T-06 | Implement `DiaSDL::InputSource` — SDL_PollEvent loop, all event translations, EKey lookup table; implement `DiaSDL::Window` + `DiaSDL::WindowFactory` — SDL_Window creation, GetSystemHandle() per-platform extraction, SDL_Init/Quit in factory | Window opens on Windows, events flow | Todo | sonnet | AC-01/AC-02/AC-07/AC-08/AC-09 |
| T-06b | Write `TestDiaSDL.cpp` — EKey lookup table round-trips + all AC-09 event type translations using synthetic SDL_Events; `DiaSDL/Testing/` following module test pattern | N tests GREEN `dia run googletest --filter="DiaSDL*"` | Todo | sonnet | TDD: write RED alongside T-06; tests go GREEN as lookup table is filled |
| T-07 | Implement `DiaSDL::Window` — all IWindow methods, GetSystemHandle() platform-conditional extraction | Window opens, resizes, closes correctly | Todo | sonnet | AC-01/AC-02; part of T-06 deliverable |
| T-08 | Update `KernelModule.h/.cpp`: replace all DiaSFML includes/types with DiaSDL equivalents; remove SFML downcast; use IInputSource::ListenForInputSources | CluicheTest runs end-to-end on Windows | Todo | sonnet | AC-07/AC-08 |
| T-09 | Delete `Dia/DiaSFML/`; remove from Cluiche.sln; remove ProjectReferences; patch render-backend.md + dia.md | Full solution builds | Todo | sonnet | AC-10; includes doc updates |
