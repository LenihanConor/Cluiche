...
[mockups/diaassetruntimeinspector.html](mockups/diaassetruntimeinspector.html) — **Ref Count Inspector** panel (bottom-left). Shows selected global asset with ref count, scope, deploy path, and per-Stage reference breakdown. Use as the visual acceptance gate after implementation.
