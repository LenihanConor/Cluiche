...
### Blueprint file I/O
- `BlueprintEditorController` loads a `.diaentity` file when a blueprint is selected in the Blueprints section
- File is parsed into an in-memory representation: list of components, each with type name and field key-value pairs
- Save writes the modified data back to the same `.diaentity` file path
...
- Right panel shows "Blueprint Identity" section:
  ...
  - File path (read-only)
  - File type badge: `.diaentity` / `.diacamera` / `.dialight`
...
| File | Change |
|---|---|
| `Dia/DiaBlueprintEditor/DiaBlueprintEditor.vcxproj` | New — static library project |
| `Dia/DiaBlueprintEditor/DiaBlueprintEditor.vcxproj.filters` | New |
| `Dia/DiaBlueprintEditor/DiaBlueprintEditorPlugin.h` | New — `IEditorPlugin` subclass |
| `Dia/DiaBlueprintEditor/DiaBlueprintEditorPlugin.cpp` | New |
| `Dia/DiaBlueprintEditor/BlueprintListController.h` | New |
| `Dia/DiaBlueprintEditor/BlueprintListController.cpp` | New |
| `Dia/DiaBlueprintEditor/BlueprintPropertyController.h` | New |
| `Dia/DiaBlueprintEditor/BlueprintPropertyController.cpp` | New |
| `Dia/DiaBlueprintEditor/BlueprintFileHandler.h` | New — `.diaentity` / `.diacamera` / `.dialight` load/save |
| `Dia/DiaBlueprintEditor/BlueprintFileHandler.cpp` | New |
| `Cluiche/Cluiche.sln` | Add `DiaBlueprintEditor.vcxproj` |

## Binding Decisions Compliance

| Decision | Compliance |
|---|---|
| SED-BP-002 | Separate extensions: `.diaentity`, `.diacamera`, `.dialight`. Implemented. |
