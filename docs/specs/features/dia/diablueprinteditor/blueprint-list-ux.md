...
- When DiaBlueprintEditor initialises and a valid `.diagame` is loaded, `get_list` queries the DiaAssetCatalogue's shared registry (via `asset_catalogue.query_by_type`) for `diaentity`, `diacamera`, `dialight`
- The private `mRegistry` + `register_catalogue_asset` handler are removed — single source of truth is the catalogue
- `blargh.diaentity` (and any other registered assets) appear in the left panel without manual intervention
...
- DiaBlueprintEditor subscribes to this notification and re-fetches the list
...
- `blueprint_editor.save` handler remains (used internally by update_field/add/remove)
