**Spec:** @docs/specs/features/dia/DiaEntityTemplateEditor/component-picker-and-defaults.md
**Status:** Done
...
- Picker data comes from the existing `entity_template_editor.get_available_components` response — already returns `typeId`, `label`, `description`.
...
- Clear button calls `entity_template_editor.update_field` with an empty-string sentinel, or a new `entity_template_editor.clear_field` handler (simpler: reuse `update_field` with `value: null` and handle on C++ side — see T6).
...
- `entity_template_editor.update_field` handler: if `data["value"].isNull()`, call a new `BlueprintMutator::ClearField(root, topKey, componentType, fieldName)` that removes the key from `"fields"`.
