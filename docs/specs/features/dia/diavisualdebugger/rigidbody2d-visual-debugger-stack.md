...
| 4 | Old debugger audit | Task 0 requires auditing `CluicheTest/` for `DiaRigidBodyVisualDebugger` usage. Where is the most likely call site? | Most likely in `CluicheTest/Levels/DummyLevel.cpp` or the main processing unit setup. Grep for `DiaRigidBodyVisualDebugger` before removing old files. |
