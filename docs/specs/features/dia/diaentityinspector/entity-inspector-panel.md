...
Implement the `DiaEntityEditorPlugin` scaffold, the entity list panel, the component field accordion (Fields tab), live field editing via `WriteField`, the `entity.inspect` WebSocket topic, and the game-side `EntityInspectSerializer`. This is the foundational feature of DiaEntityEditor — all subsequent features (query browser, mailbox monitor, watch list) build on the topic and plugin scaffold established here.
...
- The `entity.inspect` WebSocket topic is established as the data channel for all four DiaEntityEditor tabs
...
### Plugin scaffold
- `DiaEntityEditorPlugin` class exists in `Dia/DiaEntityEditor/`, inherits `IEditorPlugin`
- Plugin registers via `REGISTER_EDITOR_PLUGIN(DiaEntityEditorPlugin, "DiaEntityEditor")`
...
- `Dia/DiaEntityEditor/EntityInspectSerializer.h` (and `.cpp`) implement `SerializeEntityInspect`
...
| File | Change |
|---|---|
| `Dia/DiaEntityEditor/DiaEntityEditor.vcxproj` | New — static library project |
| `Dia/DiaEntityEditor/DiaEntityEditor.vcxproj.filters` | New |
| `Dia/DiaEntityEditor/DiaEntityEditorPlugin.h` | New — `IEditorPlugin` subclass |
| `Dia/DiaEntityEditor/DiaEntityEditorPlugin.cpp` | New |
| `Dia/DiaEntityEditor/EntityInspectorController.h` | New |
| `Dia/DiaEntityEditor/EntityInspectorController.cpp` | New |
| `Dia/DiaEntityEditor/EntityInspectSerializer.h` | New — `SerializeEntityInspect` free function |
| `Dia/DiaEntityEditor/EntityInspectSerializer.cpp` | New |
...
| `Cluiche/Cluiche.sln` | Add `DiaEntityEditor.vcxproj` |
| `Tests/GoogleTests/DiaEntityEditor/EntityInspectSerializerTests.cpp` | New |
...
| PD-002 | PU/Phase/Module architecture for app structure | `DiaEntityEditorPlugin` is a plain `IEditorPlugin` subclass with no Module/Phase/PU inheritance. Application flow lives in CluicheEditor. Compliant per SED-015. |
...
| SED-ENT-003 | `EntityInspectSerializer` is a free function, not a Domain method | Implemented as `SerializeEntityInspect` in `Dia/DiaEntityEditor/EntityInspectSerializer.h`. Domain is not touched. Compliant. |
...
| 5 | Component short tags in entity list rows | Entity list rows show "component short tags (up to 4 tags, 4 chars each)." Where do these tags come from — auto-truncated from type name, or manually registered like EntityLabelsDrawer? | Same as EntityLabelsDrawer: caller-registered via a `RegisterComponentTag(StringCRC typeId, const char* shortTag)` method on `DiaEntityEditorPlugin` (or a shared tag registry). Auto-truncation causes collisions. Registration happens in CluicheTest wiring. |
...
| 7 | `DiaEntityEditor` project dependency on `DiaEntity` | `EntityInspectSerializer` calls `IEntityInspectable` methods — so `DiaEntityEditor.vcxproj` must reference `DiaEntity.vcxproj`. Is this the correct dependency direction? | Yes. `DiaEntityEditor` depends on `DiaEntity` (for `IEntityInspectable` and `Entity` types). `DiaEntity` has no dependency on `DiaEntityEditor`. The direction is correct and acyclic. |
