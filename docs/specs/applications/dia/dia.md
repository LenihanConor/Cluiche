...
| DiaConsole | Native-window web console over DiaCLI — command browsing, typed execution results, presets, project/target context | [diaconsole.md](systems/diaconsole/diaconsole.md) |
...
| DiaMessageBus | Typed two-pass gameplay message bus — builds on DiaMailbox; `BroadcastRouter` (system fan-out) + `EntityRouter` registration point; Primary + Reaction flush passes per sim tick; 60s frame ledger ring buffer; schema browser editor tool; replaces ad-hoc observer subscriptions across gameplay systems | [diamessagebus.md](systems/diamessagebus/diamessagebus.md) |
...
| DiaEntitySpawner | Runtime entity spawning from blueprints — `SpawnRequest` one-shot API, `SpawnEmitterComponent` (rate/burst/cap/lifetime/radius policies), explicit `Despawn()`, `EntitySpawnedEvent`/`EntityDespawnedEvent` on sim stream, `EntitySpawnerModule` on SimPU | [diaentityspawner.md](systems/diaentityspawner/diaentityspawner.md) |
...
| DiaSimTime | Scalable simulation time — deterministic game clock, named clock hierarchy (SimTimeDomain), event scheduler (SimTimeScheduler), per-system LOD policy + dormancy (SimTimeRegistry), extended CPU budget (SimTimeBudget), typed PU base classes (SimModule/RenderModule/MainModule), SimTimeContext injection | [diasimtime.md](systems/diasimtime/diasimtime.md) |
...
| DiaBehaviourTree | Data-driven behaviour tree evaluator — `BehaviourTreeAsset` (JSON-loadable immutable tree definition), `BehaviourTreeSystem` (AIBudgetScheduler time-sliced Update), `BehaviourTreeComponent` (entity integration with execution cursor + decorator state), `ActionRegistry` (StringCRC→ActionFn multi-tick callbacks), built-in Sequence/Selector/Parallel/Decorator nodes, `IBehaviourTreeEventListener` observer, `IDecoratorNode` extension interface | [diabehaviourtree.md](systems/diabehaviourtree/diabehaviourtree.md) |
| DiaObjective | Data-driven gameplay goal tracking — `ObjectiveDef` (ID, classification, ConditionExpr completion/failure, reward payload, prerequisites), `ObjectiveSet` (per-objective state machine, prerequisite activation, latch semantics), `IObjectiveObserver` (Activated/Completed/Failed callbacks), `ObjectiveSetComponent` entity integration | [diaobjective.md](systems/diaobjective/diaobjective.md) |
| DiaTriggerScript | Data-driven scripted level events — `TriggerDef` (spatial/temporal/state/count trigger types), `TriggerScriptModule` (IModule on SimPU, tick-polled, per-trigger check_interval_ms), `TriggerActionRegistry` (named handler dispatch), `ITriggerActionHandler` (open extension point), `TriggerFiredEvent` on DiaStreams | [diatriggerscript.md](systems/diatriggerscript/diatriggerscript.md) |
...
| DiaGridVisibility | Per-cell visibility system — `VisibilityGroupId` group registry, tri-state cell grid (Unexplored/Revealed/Visible), LOS shadowcasting (recursive octant), `CanSee` / `GetCellState` / `GetVisibleEntities` queries | [diagridvisibility.md](systems/diagridvisibility/diagridvisibility.md) |
| DiaGridVisibilityVisualDebugger | `IDebugDomain` for DiaGridVisibility — per-cell colour overlay (Unexplored/Revealed/Visible), panel group-selector, sight-source radius circles, shadowcast boundary outline | [diagridvisibilityvisualdebugger.md](systems/diagridvisibilityvisualdebugger/diagridvisibilityvisualdebugger.md) |
...
| DiaAttribute | Data-driven gameplay attribute/stat framework — `AttributeSchema` (JSON asset: named float attributes with min/max/default), `AttributeSet` (per-entity base value + modifier stack, fixed pipeline resolution), `AttributeSetComponent` entity integration, conditional modifiers via DiaCondition, Observer change events, AI/Blackboard float accessor bridge; same idiom as DiaEconomy but a distinct composition/derivation model, no shared code | [diaattribute.md](systems/diaattribute/diaattribute.md) |
| DiaSaveGame | Generic versioned save/load system — ISaveable contract, SaveRegistry, slot management, async I/O, migration | [diasavegame.md](systems/diasavegame/diasavegame.md) |
...
| DiaDebugDomain | Visual debug domain architecture — `IDebugDomain` interface, `DiaDebugDomainRegistry` (grouped, replaces flat 64-entry list), `DiaDebugPanel` (Ultralight HTML console, left side), JS command bridge, `DiaXxxVisualDebugger` contract + `dia check`, migration of 14 existing domains, retirement of ImGui console | [diadebugdomain.md](systems/diadebugdomain/diadebugdomain.md) |
