...
Message types, serialization helpers, and data type constants are defined in the shared **DiaDebugProtocol** system (@docs/specs/applications/dia/systems/diadebugprotocol/diadebugprotocol.md). Both DiaEditor (client) and DiaDebugServer (server) include the same headers from `Dia::DebugProtocol::` namespace. GameConnectionManager uses these types directly — no editor-local protocol definitions.
...
| SED-023 | Plugins requiring game connection MUST derive from LiveConnectionPluginBase | Eliminates copy-paste of connection lifecycle (IsConnected polling, fire-if-connected at load, topic auto-subscribe/unsubscribe). Enforces consistent `<prefix>.connection_state` push and `<prefix>.get_connection_state` handler across all live plugins. | Accepted | Yes |
...
|---------|-------------|------|--------|
| Plugin Discovery | Auto-registration of editor plugins via macro, runtime discovery | @docs/specs/applications/dia/systems/diaeditor/plugin-discovery.md | Approved |
| WebSocket Client | Connection to running games, message handling, reconnection logic | @docs/specs/applications/dia/systems/diaeditor/websocket-client.md | Approved |
| CEF Integration | Chromium embedding, multi-process architecture, custom URL schemes | @docs/specs/applications/dia/systems/diaeditor/cef-integration.md | Approved |
| Docking Layout | react-mosaic integration, persistence, full-screen vs dockable | @docs/specs/applications/dia/systems/diaeditor/docking-layout.md | Approved |
| Command Palette | Search and execute DiaCLI commands from UI | @docs/specs/applications/dia/systems/diaeditor/command-palette.md | Approved |
| Output Console | Display command output, game logs, error messages | @docs/specs/applications/dia/systems/diaeditor/output-console.md | Approved |
| Live Data Visualization | Subscribe to game state, render updates, read-only monitoring | @docs/specs/applications/dia/systems/diaeditor/live-data-visualization.md | Approved |
| Undo/Redo | IEditorCommand + CommandHistory module, save-point tracking, compound commands | @docs/specs/applications/dia/systems/diaeditor/undo-redo.md | Approved |
| Home Panel | Built-in landing panel so the docking area is never empty | @docs/specs/applications/dia/systems/diaeditor/home-panel.md | Approved |
| Game Connection Panel | UI for the editor's WebSocket connection to a running game | @docs/specs/applications/dia/systems/diaeditor/game-connection-panel.md | Draft |
| Plugin Lifecycle Toolbar | Persistent toolbar strip, Plugin Browser panel, runtime load/unload/hide/show of plugins, connection status indicator | @docs/specs/applications/dia/systems/diaeditor/plugin-lifecycle-toolbar.md | Done |
| Shared File Dialog | Framework-level native file dialog service (open/save) available to all plugins via WebUIBridge | @docs/specs/applications/dia/systems/diaeditor/shared-file-dialog.md | Draft |
| Plugin Service Locator | Type-safe service locator on EditorPluginContext for inter-plugin service sharing | @docs/specs/applications/dia/systems/diaeditor/plugin-service-locator.md | Done |
| Project Context Bar | Shared `.diagame` project context for all plugins — toolbar project button, `IEditorContext::LoadProject`, `OnProjectChanged` callbacks, `--project` CLI arg, live auto-load on connect | @docs/specs/applications/dia/systems/diaeditor/project-context-bar.md | Approved |
| Editor Deep Link | `OnNavigate(instanceId)` on `IEditorPlugin` — framework calls it after `LoadPlugin` so catalogue double-clicks deliver the user to the specific asset in the target editor | @docs/specs/applications/dia/systems/diaeditor/editor-deep-link.md | Approved |
| Editor Memory | Invisible save/restore of layout, loaded plugins, and per-plugin project-scoped state on editor close/open | @docs/specs/applications/dia/systems/diaeditor/editor-memory.md | Done |
| Toast Notifications | Framework-level notification service — plugins push toasts, shell renders them uniformly | @docs/specs/applications/dia/systems/diaeditor/toast-notifications.md | Approved |
| LiveConnectionPluginBase | Shared base class for plugins that observe game connection — enforces consistent lifecycle, topic naming, auto-subscribe/unsubscribe | @docs/specs/applications/dia/systems/diaeditor/live-connection-plugin-base.md | Approved |
| Python Console | Dockable REPL plugin — execute Python interactively, run .py scripts, validate DiaEditorAPI actions | @docs/specs/applications/dia/systems/diaeditor/python-console.md | Approved |
