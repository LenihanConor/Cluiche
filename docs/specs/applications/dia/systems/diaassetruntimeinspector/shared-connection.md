...
- [x] DiaAssetRuntimeInspectorPlugin no longer owns a `GameConnectionManager` instance
...
```cpp
void DiaAssetRuntimeInspectorPlugin::OnLoad(const EditorPluginContext& context) {
...
- `Dia/DiaAssetRuntimeInspector/DiaAssetRuntimeInspectorPlugin.h` - Changed `mManager` from owned instance to pointer; added `mWasConnected`
- `Dia/DiaAssetRuntimeInspector/DiaAssetRuntimeInspectorPlugin.cpp` - Service locator query; polling; removed connect/disconnect handlers
- `Dia/DiaAssetRuntimeInspector/UI/index.html` - Removed connect bar; added disconnect overlay
...
| DiaAssetRuntimeInspector | SD-ARED-002 | Read-only, no mutation | **Compliant** — plugin only reads connection state |
| DiaAssetRuntimeInspector | SD-ARED-005 | All panels require live connection; disabled when disconnected | **Compliant** — overlay shown when disconnected; panels deactivate via `OnConnectionStateChanged(false)` |
...
| 2 | Load Order | What if DiaAssetRuntimeInspectorPlugin loads before GameConnectionEditorPlugin? | `GetService` returns nullptr; plugin shows disconnected state. In practice, GameConnectionEditorPlugin is a built-in that loads first. If load order changes, the plugin still works — it just starts disconnected and picks up the connection on next poll. |
