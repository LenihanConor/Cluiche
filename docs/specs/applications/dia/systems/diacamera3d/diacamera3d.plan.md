...
**Spec:** @docs/specs/applications/dia/systems/diacamera3d/diacamera3d.md
**Status:** Done
...
| 6 | Implement `Follow3D` + `SmoothDamp3D` behaviours; self-register on startup | Tests in `TestBehaviours3D.cpp` — target tracking, smoothing convergence | Done | sonnet | 6194 tests pass |
| 7 | Implement `BoundsClamp3D` (AABB clamp, no-op for zero-volume) + `ScreenShake3D` (trauma-based additive position + orientation, attach last) | Tests — clamp boundary, shake decays to zero | Done | sonnet | 6194 tests pass |
