...
**Spec:** @docs/specs/applications/dia/systems/diaapplicationflowinspector/diaapplicationflowinspector.md
**Status:** Done
...
| # | Task | Test | Status | Model | Notes |
|---|------|------|--------|-------|-------|
| 32 | Register `stream.{id}.current_size` gauge per EventStreamStore; set after every `SendInternal()` call | `TestStreamTelemetry`: create stream cap=8, send 5 → gauge reads 5; consume 3 → gauge reads 2 | Done | sonnet | Add to `EventStreamStore` constructor (register) + `SendInternal` (set). ~5 lines. Uses existing `MetricRegistry::Instance()`.; Gauge registered in EventStreamStore constructor; set after SendInternal() fan-out. DiaObservation added as ProjectReference to DiaStreams.vcxproj. |
| 33 | Add `mDropsTotal` counter to `EventStreamStore`; increment on each overflow (all policies); register as `stream.{id}.drops_total` | `TestStreamDrops`: send cap+1 with DropOldest → counter=1; send cap+2 → counter=2; no overflow → counter=0 | Done | sonnet | In overflow switch cases (3 locations). ~5 lines.; Counter registered; Increment() on kDropOldest/kDropNewest/kBlock overflow branches. kFailLoud excluded (hard error, not a drop). |
| 34 | Wrap `ProcessingUnit::Update()` body with QPC; store result in `mLastTickMs` (float); register `pu.{id}.last_tick_ms` gauge; set after tick completes | `TestPUTiming`: create PU, call Update(0.016f), assert `mLastTickMs > 0.0f` and gauge value matches | Done | sonnet | ~10 lines. Existing `DIA_PROFILE_SCOPE` already times this; QPC adds authoritative gauge for broadcast. Use `Dia::Core::HighResTimer` if available, else raw QPC.; mLastTickMs + mMetricLastTickMs added. std::chrono wraps Update() body. Gauge set after tick. DiaApplicationFlow builds clean. |
| 35 | Add `DIA_LOG_WARNING` in PU::Update when `mLastTickMs > targetPeriodMs` (over-budget) | `TestPUTiming`: set frequency to 1000Hz (1ms budget), Update with artificial 5ms work → log emitted | Done | haiku | Single line: `DIA_LOG_WARNING("pu", "pu.over_budget id=%s tick_ms=%.1f target_ms=%.1f", ...)`; DIA_LOG_WARNING emitted when mLastTickMs > targetMs. One log per over-budget tick. No throttle needed for dev tool. |
| 36 | Include `currentSize`, `dropsTotal`, `lastTickMs` in existing `ObservationBridge::OnSnapshot()` broadcast path (metric snapshots already broadcast gauge/counter values) | `TestInspectorPlugin` (integration): connect mock client, subscribe to `observation.metric`, assert fields present in JSON payload | Done | sonnet | Verify no new code needed — registered metrics should auto-appear in snapshot. If not, add to snapshot loop.; No code needed. MetricRegistry::Snapshot() iterates all registered metrics unconditionally. ObservationBridge serializes by entry.kind, no name filter. |
