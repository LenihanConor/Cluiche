...
**Status:** Approved  
**Parent Application:** @docs/specs/applications/dia/dia.md  
...
|---------|-------------|--------|
| `sanitizer-configs` | Debug-Asan and Debug-Ubsan build configurations; `dia check --tool=sanitizer` | [sanitizer-configs.md](sanitizer-configs.md) Approved |
| `cppcheck-integration` | Cppcheck install, `.cppcheck-suppressions.xml`, `dia check --tool=cppcheck`, SARIF output | [cppcheck-integration.md](cppcheck-integration.md) Approved |
| `dia-diagnose-loop` | Agentic Claude fix loop with context gather, apply, verify, stage/revert | [dia-diagnose-loop.md](dia-diagnose-loop.md) Approved |
| `ci-gate` | `dia pipeline` static-analysis stage; baseline comparison; fail on new error findings | [ci-gate.md](ci-gate.md) Approved |
