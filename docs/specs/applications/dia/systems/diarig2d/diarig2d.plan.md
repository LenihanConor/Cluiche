...
**Spec:** @docs/specs/applications/dia/systems/diarig2d/diarig2d.md  
**Status:** Done  
**Started:** 2026-05-01  
...
| 2 | **Bone** — `Bone.h`: Bone struct with defaults, in `Dia::Rig2D::` namespace | [flat-skeleton.md](flat-skeleton.md) | Done | Extended with MetadataValue variant + MetadataEntry bag (SetMetadata/FindMetadata) |
| 3 | **Skeleton** — `Skeleton.h` / `Skeleton.cpp`: SkeletonDef struct, Skeleton class with constructor validation (topology, single root, unique names, max bones), bone length computation, FindBoneIndex, GetRequiredBoneIndex | [flat-skeleton.md](flat-skeleton.md) | Done | kMaxBones = 128 (reduced from 256 to avoid stack overflow) |
| 4 | **BoneTransform + Pose** — `BoneTransform.h`, `Pose.h` / `Pose.cpp`: Pose class with bind pose init, GetLocalTransform, ComputeWorldTransforms (FK with SRT, negative scale propagation) | [pose-blending.md](pose-blending.md) | Done | Uses inline Affine2x3 helpers in anonymous namespace |
| 5 | **BlendPoses** — `BlendPoses.h` / `BlendPoses.cpp`: shortest-arc rotation lerp, clamped t, bone count validation | [pose-blending.md](pose-blending.md) | Done | |
| 6 | **JSON loader** — `ISkeletonLoader.h` interface + `SkeletonJson.h/cpp` default impl. Bool return for data errors, Rig2D channel logging. | [json-skeleton-definitions.md](json-skeleton-definitions.md) | Done | Loader behind ISkeletonLoader interface per user request; metadata in explicit "metadata" JSON block |
| 7 | **Test utilities** — `Testing/SkeletonBuilders.h/cpp`, `Testing/PoseComparison.h/cpp`. MakeSimpleChain, MakeHumanoid, MakeBranching, PosesAreEqual, BoneTransformsAreEqual. | [test-utilities.md](test-utilities.md) | Done | Ships with DiaRig2D library |
| 8 | **Skeleton component** — `SkeletonComponent.h` / `SkeletonComponent.cpp`: IComponent wrapper owning Skeleton + Pose, DynamicComponentFactory registration, ResetToBindPose | [skeleton-component.md](skeleton-component.md) | Done | COMPONENT_DECLARATION(0x52494730) |
...
| 10 | **Debug renderer project** — `Dia/DiaRig2DVisualDebugger/` with own vcxproj, refs DiaRig2D + DiaGraphics | [skeleton-debug-renderer.md](skeleton-debug-renderer.md) | Done | GUID {F5A6B7C8-D9E0-1234-BCDE-56789ABCDEF1} |
| 11 | **Debug renderer implementation** — `VisualDebugger.h/cpp`: bones as white lines, root as green circle, joints as yellow circles. On/off + bone names toggle. | [skeleton-debug-renderer.md](skeleton-debug-renderer.md) | Done | Text rendering deferred (FrameData may not support text primitives) |
