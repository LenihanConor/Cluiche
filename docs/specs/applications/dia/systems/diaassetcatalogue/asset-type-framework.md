...
DiaAssetCatalogue owns the descriptors — what asset types exist and what their data shape looks like. Downstream systems (DiaAssetPipeline via DiaPipeline `build-assets`) own what to do with them: validation execution, build step registration, and deployment.
...
8. Ships with descriptors for all 8 settled taxonomy types: Texture, Mesh/Sprite, Audio, Config, Entity Definition, Stage, UI Definition, Folder
9. Registering a new game-specific asset type = constructing and registering one descriptor. No changes needed in DiaAssetCatalogue or downstream systems.
...
```cpp
// Called once at startup to register the 8 settled types
...
| `folder` | Folder | `*.folder/` | FolderAsset |

The TypeDefinitions for these built-in types define the base schema (what fields a Stage etc. must have). Folder assets have no JSON schema — they are directories. Game-specific extensions add fields to their own types, not to these.
...
| 5 | Define built-in taxonomy TypeDefinitions | C++ structs for the 8 asset types (TextureAsset, SpriteAsset, etc.) registered with TypeSystem. Fields use TypeVariableAttributeRequired and TypeVariableAttributeAssetReference where applicable. FolderAsset has no JSON schema — it is a directory. |
| 6 | Implement RegisterBuiltInAssetTypes | Single function registering all 8 descriptors |
...
|------------|----------------------|
| DiaAssetCatalogue — JSON Definition Loader (Feature 1) | LoadResult/LoadError types. TypeVariableAttributeRequired (added in Feature 1's Task 4). |
...
| File | Action |
|------|--------|
| `Dia/DiaAssetCatalogue/AssetTypeDescriptor.h` | Create — descriptor struct |
| `Dia/DiaAssetCatalogue/AssetTypeRegistry.h` | Create — registry class header |
| `Dia/DiaAssetCatalogue/AssetTypeRegistry.cpp` | Create — registry implementation |
| `Dia/DiaAssetCatalogue/BuiltInAssetTypes.h` | Create — built-in taxonomy TypeDefinitions and registration |
| `Dia/DiaAssetCatalogue/BuiltInAssetTypes.cpp` | Create — implementation |
...
| PD-008 | Directory.Build.props | **Compliant.** DiaAssetCatalogue.vcxproj and DiaCore.vcxproj inherit centralized settings. |
| PD-009 | Generated output under Cluiche/out/ | **Not applicable.** No generated output. |
| AD-001 | YAML frontmatter module docs | **Compliant.** DiaAssetCatalogue module doc already created in Feature 1. |
| AD-002 | No STL in public APIs | **Compliant.** Same as PD-004. |
| AD-003 | Namespace Dia::\<Module\>:: | **Compliant.** All code under `Dia::AssetCatalogue::`. New DiaCore attribute under `Dia::Core::Type::`. |
...
| 2 | Taxonomy | Should the built-in TypeDefinitions (TextureAsset, SpriteAsset, etc.) be minimal stubs or fully-fleshed schemas? | Minimal stubs defining the essential fields that the pipeline and runtime will need (e.g. StageAsset has a name and display name — asset membership is defined by `contains` relationships in the catalogue, not listed in the stage file (SD-CAT-012); EntityAsset has a components array). FolderAsset has no JSON schema — it is a directory. Game-specific fields go in game-specific types. |
...
| 6 | Scope | The 8 built-in TypeDefinition structs (TextureAsset, etc.) are non-trivial to design — their fields determine what the pipeline validates and what the runtime expects. Should their exact field definitions be deferred to the pipeline/loader specs? | Yes — Feature 2 defines minimal stub structs with the obvious structural fields (e.g. StageAsset.mName, StageAsset.mDisplayName; EntityAsset.mComponents). Stage asset membership is defined by `contains` relationships (SD-CAT-012), not by fields in the stage file. The pipeline and loader specs may extend these or add constraints. The stubs are the contract that "these fields will exist." |
