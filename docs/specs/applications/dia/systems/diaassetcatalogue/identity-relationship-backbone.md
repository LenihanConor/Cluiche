...
| System | DiaAssetCatalogue | @docs/specs/applications/dia/systems/diaassetcatalogue/diaassetcatalogue.md |
...
This is the backbone that the pipeline (DiaAssetPipeline), runtime loader (DiaAssetRuntime), and editor (DiaAssetCatalogueEditor) all read from. It is pure data model — it does not load asset content, run validation, or execute build steps.

## Problem

Without a central registry, every system that touches assets must independently discover and track what exists and how it relates. The pipeline can't know what to build, the editor can't show relationships, and the runtime loader can't resolve Stage → Asset chains. "What uses this texture?" is unanswerable without scanning the entire project. A single catalog with bidirectional relationships solves all of these.

## Acceptance Criteria

1. `AssetRecord` stores: composite StringCRC ID (`type.name`), asset type ID (StringCRC), source path (FilePath), content hash (uint32), metadata tags (DynamicArrayC of StringCRC), status (enum: active/deprecated/draft), scope (enum: global/stage), scope stage name (StringCRC, only meaningful when scope is `kStage`), and a list of forward references (DynamicArrayC of typed relationship edges). Scope and stage name comply with SD-CAT-007.
...
4. Registry is populated by external callers — the pipeline discover phase or manifest deserialization. DiaAssetCatalogue does not scan directories itself.
...
namespace Dia::AssetCatalogue
{
    ...
    struct AssetRecord
    {
        ...
        AssetStatus mStatus;
        AssetScope mScope;                      // SD-CAT-007: global or stage
...
```json
{
    ...
    "assets": [
        ...
        },
        {
            ...
            "type": "config",
            "source_path": "Raw/Config/ship_stats.config.json",
            "content_hash": 7261839450,
            "status": "active",
            "scope": "global",
            "stage_name": "",
            "tags": ["gameplay"],
...
namespace Dia::AssetCatalogue
{
    ...
    {
    public:
        CatalogueManifestSerializer(const JsonDefinitionLoader& loader);
...
| # | Task | Description |
|---|------|-------------|
| 1 | Implement AssetRecord and supporting types | AssetRecord struct, AssetStatus enum, AssetScope enum, RelationshipEdge struct, built-in relationship type constants (kUses, kContains, kConfiguredBy, kDependsOn) |
...
| 5 | Implement CatalogueManifestSerializer | LoadManifest (reads JSON via JsonDefinitionLoader, populates registry, resolves single-level includes, rejects duplicates). SaveManifest (serializes registry to JSON). |
...
|------------|----------------------|
| DiaAssetCatalogue — JSON Definition Loader (Feature 1) | JsonDefinitionLoader for manifest deserialization, LoadResult/LoadError for error reporting |
| DiaAssetCatalogue — Asset Type Framework (Feature 2) | AssetTypeRegistry for validating that asset type IDs in records match registered types |
...
| File | Action |
|------|--------|
| `Dia/DiaAssetCatalogue/AssetRecord.h` | Create — record struct, status enum, relationship edge |
| `Dia/DiaAssetCatalogue/RelationshipTypes.h` | Create — built-in relationship type StringCRC constants |
| `Dia/DiaAssetCatalogue/AssetRegistry.h` | Create — registry class header |
| `Dia/DiaAssetCatalogue/AssetRegistry.cpp` | Create — registry implementation |
| `Dia/DiaAssetCatalogue/RelationshipIndex.h` | Create — relationship index header |
| `Dia/DiaAssetCatalogue/RelationshipIndex.cpp` | Create — relationship index implementation (lazy reverse build) |
| `Dia/DiaAssetCatalogue/CatalogueManifestSerializer.h` | Create — manifest read/write header |
| `Dia/DiaAssetCatalogue/CatalogueManifestSerializer.cpp` | Create — manifest implementation with include support |
...
| PD-003 | Component-based entities | **Compliant.** Entity Definition is a registered asset type. Its references to component types use StringCRC (PD-001). Component instantiation is DiaAssetRuntime's job. |
...
| PD-008 | Directory.Build.props | **Compliant.** DiaAssetCatalogue.vcxproj inherits centralized settings. |
...
| AD-001 | YAML frontmatter module docs | **Compliant.** DiaAssetCatalogue module doc created in Feature 1. |
| AD-002 | No STL in public APIs | **Compliant.** Same as PD-004. |
| AD-003 | Namespace Dia::\<Module\>:: | **Compliant.** All code under `Dia::AssetCatalogue::`. |
