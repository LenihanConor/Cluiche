...
|---------|-------------|------|--------|--------|
| ndjson-tailer | C++ file tailer that polls and parses the NDJSON pipeline log into PipelineEvent structs | [ndjson-tailer.md](ndjson-tailer.md) | 3 days | Done |
| pipeline-panel-ui | React/TypeScript dockable panel — stage timeline, log drill-down, run summary | [pipeline-panel-ui.md](pipeline-panel-ui.md) | 5 days | Done |
| build-trigger | DiaAPI commands to start/cancel pipeline from editor; auto-attach tailer to new run | [build-trigger.md](build-trigger.md) | 2 days | Done |
| run-history | Store and browse last 10 pipeline runs with summary data | [run-history.md](run-history.md) | 2 days | Done |
| sub-step-visibility | Emit OnStep* events from DiaCLI stage handlers; parse and render in C++ tailer and React UI | [sub-step-visibility.md](sub-step-visibility.md) | 1 day | Done |
| pipeline-build-ux | Game-scoped build panel: ghost stages, timing bars, failure inline, stdout capture, Build/Launch split-button | [pipeline-build-ux.md](pipeline-build-ux.md) | 4 days | Approved |
...
**Plan:** @docs/specs/applications/dia/systems/diapipelineeditor/diapipelineeditor.plan.md
