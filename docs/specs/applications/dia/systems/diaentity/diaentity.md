...
| foundation | `Domain`, `Entity`, `IComponent` base, component type-pools (one `HandlePool<T>` per component type), entity create/destroy lifecycle, basic `GetComponent`/`HasComponent`, end-of-frame structural change application. | [foundation.md](foundation.md) | Approved |
| reflection | `DIA_COMPONENT` macro, `FIELD` macro, `ComponentTypeDesc`, `ComponentRegistry`, JSON load/save thunks, schema version field. Per Decision 14. | [reflection.md](reflection.md) | Approved |
| blueprint-loader | `JsonBlueprintLoader` interface + concrete impl, versioned JSON schema with separate references block, asset-trigger callback into components. Per Decision 11. | [blueprint-loader.md](blueprint-loader.md) | Approved |
| component-deps-and-refs | `REQUIRES` macro with hard validation at attach, `EntityRef<TComponent>` typed reference slots, two-pass blueprint resolution (instantiate then patch references). Per Decisions 4, 7. | [component-deps-and-refs.md](component-deps-and-refs.md) | Approved |
| hierarchy | `ParentComponent`, `ChildBufferComponent`, `Hierarchy::QueueSetParent`/`QueueDestroySubtree`, cycle rejection in debug, destroy-subtree default cascade. Per Decision 15. | [hierarchy.md](hierarchy.md) | Approved |
| mailbox-router | `EntityRouter`, address encoding (`MakeEntityAddress` etc.), auto-registration of router with domain's mailbox, resolve implementations for all four kinds (Entity/All/ComponentType/Self). | [mailbox-router.md](mailbox-router.md) | Approved |
| query-system | `Domain::Query<...>`, signature-keyed query cache, end-of-frame invalidation/rebuild. Per Decision 10. | [query-system.md](query-system.md) | Approved |
| editor-inspection | `IEntityInspectable` implementation on Domain, reflection-driven `ReadField`/`WriteField`, mailbox log accessor. Tiers (a) + (b) per Decision 17. | [editor-inspection.md](editor-inspection.md) | Approved |
| update-loop | `Domain::Update(dt)` walks DoUpdate-opted components in registration order, then entity-index order. Per Decision 6. | [update-loop.md](update-loop.md) | Approved |
| module-and-build | `diaentitytemplate.vcxproj`, registration in `Cluiche.sln`, `dia.entity.architecture.module.md` YAML, dependency edge in `dia_modules.py`. | [module-and-build.md](module-and-build.md) | Approved |
