...
| System | @docs/specs/applications/dia/systems/diamailbox/diamailbox.md | **typed-queue** |
...
| 3 | Descriptor registry capacity | `DynamicArrayC<TypedQueueDescriptor*, kMaxTypes>` — what is `kMaxTypes` and is 32 enough? | 32 is the v1 default (matches typical module-per-stage type counts). If a consumer registers more than 32 types, `RegisterType` returns `false` after emitting `DIA_ASSERT`. `kMaxTypes` is a compile-time constant in `Mailbox.h` — easy to raise without API churn. Real consumers (diaentitytemplate) will size it based on their message type count when they spec out their usage. |
