...
|---|---------|------|--------|-------|-------|
| 1 | module-and-build | [module-and-build.md](module-and-build.md) | Done | haiku | Empty vcxproj skeleton + sln registration + YAML doc. Must be first. |
| 2 | address-and-types | [address-and-types.md](address-and-types.md) | Done | haiku | `MailboxTypes.h` + `MailboxTypes.cpp` + tests. 9/9 GREEN. Note: `Address` is not trivially copyable — `StringCRC` has user-defined copy ctor; AC9 corrected in spec. |
| 3 | typed-queue | [typed-queue.md](typed-queue.md) | Done | sonnet | `Mailbox.h/.cpp` ring buffer + overflow + tests. 15/15 GREEN. Note: MSVC macro comma issue fixed with `((...))` parens; `DIA_LOG_WARNING` replaced with `Dia::Core::Log::OutputLine` via warn callback. |
| 4 | subscriptions | [subscriptions.md](subscriptions.md) | Done | sonnet | `Subscription.h` + HandlePool wiring + tests. 13/13 GREEN. Note: `SubscriptionHandle` defined in `Mailbox.h` (not Subscription.h) to avoid circular include; stores non-owning pool pointer for generation-aware IsValid(). |
| 5 | routers | [routers.md](routers.md) | Done | sonnet | `IMailboxRouter.h` + `Resolve<T>` + `MockRouter` + tests. 11 router tests GREEN. 49/49 total. Note: MailboxFixture naming — used existing MailboxTestFixture; no new Testing::MailboxFixture created. |
