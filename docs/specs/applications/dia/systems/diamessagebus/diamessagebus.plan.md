...
| 3 | [core-bus](core-bus.md) (two-pass flush) — `Pass` enum, Reaction pass drain, re-entrancy assert (Debug) / silent drop (Release) per SD-MBX2-001 res. #1 | `TestDiaMessageBusFlush*` | Done | sonnet | Prereq: 2; same feature spec (Q1), split out as its own task — hardest single piece; Reaction sweep + Post re-entrancy guard implemented; DIA_ASSERT firing verified via g_pAssertFunc recorder. 25/25 tests green, 53/53 Mailbox regression green. KNOWN LIMITATION (flagged to user, not fixed): reaction delivery is order-dependent on RegisterType order between posting type and target type -- a message can be silently consumed as Primary within the same sweep before the Reaction sweep runs, if the target type's registration index is after the poster's. Needs a real design decision (pre-sweep snapshot or buffer-then-dispatch restructuring) -- candidate for a new Open Design Question in core-bus.md. |
