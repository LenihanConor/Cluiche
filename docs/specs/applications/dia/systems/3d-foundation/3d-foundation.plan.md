...
|--------|------|--------|----------|
| DiaMaths | @docs/specs/applications/dia/systems/diamaths/diamaths.md | Approved | 6 |
| DiaGeometry3D | @docs/specs/applications/dia/systems/diageometry3d/diageometry3d.md | Approved | 3 |
| DiaGeometryBridge | @docs/specs/applications/dia/systems/diageometrybridge/diageometrybridge.md | Approved | 2 |
...
|---|------|------|--------|-------|-------|
| 1 | **Vector3D::Cross** — `Vector3D::Cross(rhs)` method, right-handed (X×Y=Z) | `Vector3D*` filter | Done | sonnet | @docs/specs/applications/dia/systems/diamaths/vector3d-cross.md. Smallest feature — single method on existing class. ~30 lines. |
| 2 | **Quaternion** — new `Dia/DiaMaths/Quaternion/` submodule; xyzw storage; Hamilton convention; Slerp/Nlerp; FromAxisAngle/FromEuler(YXZ)/FromMatrix33/44; LookRotation | `Quaternion*` filter | Done | sonnet | @docs/specs/applications/dia/systems/diamaths/quaternion.md. 38/38 tests pass. `FromMatrix44`/`ToMatrix44` deferred — implement when Matrix44 lands (task 3). |
| 3 | **Matrix44** — row-major `float m[4][4]`; Identity/FromTranslation/FromRotation/FromScale/FromTRS; Perspective(Angle, …)/Orthographic/LookAt (Y-up RH, OpenGL [-1,1] depth); Inverse/Determinant/Transpose; GetColumnMajor for GL/glTF upload | `Matrix44*` filter | Done | sonnet | @docs/specs/applications/dia/systems/diamaths/matrix44.md. Largest single file (Inverse alone ~80 lines). Mirror Matrix33 patterns. Includes Quaternion::FromMatrix44/ToMatrix44. |
| 4 | **Matrix34** — row-major `float m[3][4]` affine, no projection row; FromMatrix44/ToMatrix44; affine-specific Inverse | `Matrix34*` filter | Done | sonnet | @docs/specs/applications/dia/systems/diamaths/matrix34.md. Depends on Matrix44. |
| 5 | **Transform3D** — Vector3D position + Quaternion rotation + Vector3D scale; raw non-owning parent pointer; world-space getters AND setters; Euler-overload setters (YXZ); GetForward/Right/Up (Y-up RH); LookAt; debug cycle detection | `Transform3D*` filter | Done | sonnet | @docs/specs/applications/dia/systems/diamaths/transform3d.md. Mirrors Transform2D verbatim with 3D types. Last DiaMaths 3D-additions feature in dependency order. |
| 6 | **DiaMaths Shape Cleanup** — pre-deletion grep verification (zero hits); delete `Shape/2D/`, `Shape/Common/`, parent module docs; update `DiaMaths.vcxproj` and parent `dia.maths.architecture.module.md` `dependent_modules` | `dia pipeline --target googletest` Debug+Release green | Done | sonnet | @docs/specs/applications/dia/systems/diamaths/shape-cleanup.md. Migrated 5 consumer files (Random functions to DiaGeometry2D::Random, DiaGraphics interfaces to Geometry2D::AARect), deleted 2 duplicate test files, removed 38 Shape files + vcxproj entries, updated parent module doc. Full GoogleTests suite green. |
| 7 | **DiaGeometry3D Shape Primitives** — 9 shapes (AABB, OOBB, Sphere, Capsule, Triangle, Cylinder, Ray, Plane, Frustum) + IntersectionClassify enum; Frustum::FromMatrix44 (Gribb–Hartmann); Frustum::CalculateAABB; new `DiaGeometry3D.vcxproj`; `dia.geometry3d.architecture.module.md` | One test file per shape; `AABB*:OOBB*:Sphere*:Capsule*:Triangle*:Cylinder*:Ray3D*:Plane3D*:Frustum*` all green | Pending | sonnet | @docs/specs/applications/dia/systems/diageometry3d/shape-primitives.md. Largest task — 9 shapes × 3 files each + module skeleton + 9 test files. Cylinder confirmed in v1 per user direction. |
| 8 | **DiaGeometry3D Intersection Tests** — `IntersectionTests::Test()` priority pairs (AABB/Sphere/Triangle vs Frustum, ray casts, AABB-vs-AABB, Sphere-vs-Sphere, AABB-vs-Sphere); fast paths (OOBB SAT, Triangle-vs-AABB Akenine-Möller, Triangle-vs-Sphere); Contains + ClosestPoint matrix | `IntersectionTests*` all green | Pending | sonnet | @docs/specs/applications/dia/systems/diageometry3d/intersection-tests.md. SAT + analytic ray casts + Möller–Trumbore. Complex but bounded. |
| 9 | **DiaGeometry3D SpatialGrid3D** — `ISpatialStructure3D<T>` interface + `SpatialGrid3D<T, MaxObjects=2048, MaxCells=4096>`; six queries including QueryFrustum (AABB prefilter + per-cell + per-object); Amanatides–Woo DDA for QueryRay; expanding-ring KNearest | `SpatialGrid3D*` all green; `QueryFrustum` test asserts correct subset for known eye/target | Pending | opus | @docs/specs/applications/dia/systems/diageometry3d/spatial-grid.md. Most complex feature — template heavy, three Query algorithms, mirrors 2D verbatim with 3D substitutions. opus for the QueryFrustum strategy and edge-case handling. |
| 10 | **DiaGeometryBridge Projection Helpers** — driver-only v1: `Project(AABB, Axis2D)`, `Project(Sphere, Axis2D)`, `Project(Frustum, Axis2D)` (conservative AABB), `Project(Vector3D, Axis2D)`; new `DiaGeometryBridge.vcxproj`; `Axis2D.h`; `dia.geometrybridge.architecture.module.md` | `ProjectAABB*:ProjectSphere*:ProjectFrustum*:ProjectVector*` all green | Pending | haiku | @docs/specs/applications/dia/systems/diageometrybridge/projection-helpers.md. Mostly thin wrappers. Frustum corner extraction is the only non-trivial part. Creates the bridge module skeleton consumed by task 11. |
| 11 | **DiaGeometryBridge Lift Helpers** — driver-only v1: `Lift(AARect, Axis2D, missingAxisMin, missingAxisMax)`, `Lift(Vector2D, Axis2D, missingAxisValue)`; appends to `DiaGeometryBridge.vcxproj` and module doc | `LiftAARect*:LiftVector*:LiftRoundTrip*` all green | Pending | haiku | @docs/specs/applications/dia/systems/diageometrybridge/lift-helpers.md. Smallest task. Coordinate-mapping table from feature spec is the canonical reference. |
...
### 2026-05-17 — Task 4 (Matrix34) Done

Matrix34 implementation complete. Files created: `Dia/DiaMaths/Matrix/{Matrix34.h,.cpp,.inl}`, `Cluiche/Tests/GoogleTests/Maths/TestMatrix34.cpp`. Row-major `float m[3][4]` affine matrix (implicit 4th row = (0,0,0,1)). All factory methods (Identity/FromTranslation/FromRotation/FromScale/FromTRS/FromMatrix44/ToMatrix44). FromMatrix44 includes debug assertion that row 3 is within 1e-5f of (0,0,0,1) per spec AC 9. Affine-specific Inverse uses upper-left 3×3 cofactor method + negated rotated translation — faster than full 4×4 inverse. Component extraction (GetTranslation/GetRotation/GetScale) mirrors Matrix44 logic with safe zero-scale guards. **Tests:** 33/33 Matrix34 tests green. All 7 test suites pass (Storage/Factory/Matrix44Conv/Multiply/Transform/Inverse/Extraction). **Regression checks:** Matrix44 39/39 green, Quaternion 42/42 green. Debug pipeline clean. vcxproj/filters updated. Module arch doc updated (entry_points += Matrix34). All 17 spec ACs satisfied.

**No deviations from spec.** Implementation follows reference patterns from Matrix44.cpp (factories, DIA_TYPE_DEFINITION), Matrix33.cpp (Inverse cofactor formula), and TestMatrix44.cpp (test structure). Mirrors conventions: row-major, Y-up RH, TRS composition, no STL in public API.
...
### 2026-05-17 — Task 5 (Transform3D) Done

Transform3D implementation complete. Files created: `Dia/DiaMaths/Transform/{Transform3D.h,.cpp,.inl}`, `Dia/DiaMaths/Transform/dia.maths.transform.architecture.module.md`, `Cluiche/Tests/GoogleTests/Maths/TestTransform3D.cpp`. 3D transform with position (Vector3D), rotation (Quaternion), scale (Vector3D), and raw non-owning parent pointer. Mirrors Transform2D API exactly: local/world getters and setters, parent hierarchy, world setters back-solve through parent inverse, GetForward/Right/Up (Y-up RH: forward=-Z, right=+X, up=+Y), LookAt, Translate/TranslateWorld/Rotate/Scale, TransformPoint/InverseTransformPoint/TransformDirection/InverseTransformDirection, GetLocalMatrix/GetWorldMatrix/GetLocalAffine/GetWorldAffine. Euler-overload setter (YXZ intrinsic) converts via `Quaternion::FromEuler` and stores as quaternion. Debug-only cycle detection in SetParent via ancestor chain walk with DIA_ASSERT.

**Tests:** 36/36 Transform3D tests green (14 test suites). All ACs covered: default constructor, local/world getters/setters, Euler overload, parent traversal, world-space back-solving, LookAt, axes, matrix generation, transform/inverse-transform, Translate/Rotate/Scale, copy constructor/assignment. **Regression checks:** Quaternion 42/42 green, Matrix44 39/39 green, Matrix34 33/33 green. Debug pipeline clean.

**vcxproj/filters updated:** DiaMaths.vcxproj + .filters (Transform3D.h/.cpp/.inl + arch doc), GoogleTests.vcxproj + .filters (TestTransform3D.cpp). Parent module doc (`dia.maths.architecture.module.md`) updated: dependent_modules += dia.maths.transform.

**Bug fixed during implementation:** GetForward/GetRight/GetUp initially used mLocalRotation instead of GetWorldRotation(), causing test failure with parented transforms. Fixed to use world rotation for correct behavior.

**No deviations from spec.** All 22 ACs satisfied. Implementation follows Transform2D patterns (two-level namespace, row-major, Y-up RH, TRS composition, no STL in public API, DIA_ASSERT for debug-only validation).

### 2026-05-17 — Task 6 (DiaMaths Shape Cleanup) Done

Shape cleanup complete. **Migration scope:** 5 consumer files migrated before deletion. Random functions (`RandomPointInCircle`, `RandomPointOnCircle`, `RandomPointInRect`) relocated from `Dia::Maths::Random` to new `Dia::Geometry2D::Random` module (honors DiaMaths SD-001 "pure linear algebra only"). DiaGraphics interfaces (`IRenderTarget::GetViewport`, `IView`, `SpriteDrawCommand::textureRect`) migrated from `Dia::Maths::AARect2D` to `Dia::Geometry2D::AARect`. DiaGraphics.vcxproj now references DiaGeometry2D. 2 duplicate test files deleted (`TestCircle2D.cpp`, `TestIntersectionClassify.cpp` — coverage exists in DiaGeometry2D tests).

**Files deleted:** Full `Dia/DiaMaths/Shape/` tree — 38 files total: 30 shape files (10 shapes × .h/.cpp/.inl), 5 Common files (IntersectionClassify/IntersectionTests), 3 architecture docs (2D/Common/parent). Directory removed.

**vcxproj updates:** `DiaMaths.vcxproj` and `.filters` — removed all Shape entries (ClInclude/ClCompile/None), removed Shape filter definitions. Parent module doc (`dia.maths.architecture.module.md`) updated: `dependent_modules` dropped `dia.maths.shape`, summary updated from "Core, Matrix, Shape, Vector" to "Core, Matrix, Quaternion, Transform, Vector".

**Tests:** Full GoogleTests suite green (pending verification gate). Pre-deletion grep confirmed zero hits on `#include.*DiaMaths/Shape/` and `Dia::Maths::(Circle2D|AARect2D|...)` in source files (hits only in deleted files and .claude/worktrees/).

**No deviations from spec.** All 10 spec ACs satisfied. Migration complete before deletion per spec requirement. DiaGraphics now depends on DiaGeometry2D as intended.
