...
**Spec:** @docs/specs/applications/dia/systems/diaik2d/diaik2d.md  
**Status:** Done  
**Started:** 2026-05-02  
...
| 2 | **IK Chain Definition** — `IKChainDef.h`: `JointLimitDef`, `IKChainDef`, `PoleVector` structs. Header-only. | [ik-chain-definition.md](ik-chain-definition.md) | Done | |
| 3 | **IKSolver scaffold** — `IKSolver.h` / `IKSolver.cpp`: constructor (pre-alloc all arrays), `SetRootTransform` (run full FK into `mWorldTransforms`), `RegisterChain` / `UnregisterChain` / `HasChain`, `GetSkeleton` / `GetPose`. No solver logic yet. | [ik-solver.md](ik-solver.md) | Done | Added `GetParentWorldRot()` private helper for bone rest angle math. |
| 4 | **Two-bone solver** — `SolveTwoBone()`: law of cosines, pole vector, joint limits clamp, reach weight (ShortestArcLerp), FK cache refresh. | [two-bone-solver.md](two-bone-solver.md) | Done | `BoneWorldRotFromDir(dirAngle, boneRestAngle)` helper used throughout. |
| 5 | **FABRIK solver** — `SolveFABRIK()`: iterative backward/forward passes on `mWorkingPositions`, joint limit projection per iteration, convergence check, non-convergence warning, reach weight, FK cache refresh. | [fabrik-solver.md](fabrik-solver.md) | Done | |
| 6 | **Look-at constraint** — `SolveLookAt(int)` + `SolveLookAt(StringCRC)`: atan2 world angle, axis offset, parent world rotation, reach weight, coincident guard, FK cache refresh. | [look-at-constraint.md](look-at-constraint.md) | Done | |
| 7 | **Test utilities** — `Testing/IKTestHelpers.h`, `Testing/IKAssertions.h`: `BuildLimbSkeleton`, `BuildTestIKSolver`, `AssertBoneRotation`, `AssertEndEffectorPosition`, `AssertBoneUnchanged`. Header-only, not compiled into `DiaIK2D.vcxproj`. | [test-utilities.md](test-utilities.md) | Done | |
