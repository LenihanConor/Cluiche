...
**Status:** In Progress — **Phase 1 (Foundation) Done** (2026-09-02): all of Tasks 1.0-1.8 plus C.1 complete, reviewed, and verified (1386 tests + `cluichetest` clean on `Development`). The hard-cutover window (Tasks 1.3-1.6) landed on a short-lived branch (`diasimtime/typed-modules-cutover`) and fast-forward-merged cleanly. Phases 2-4 (SimTimeDomain, SimTimeScheduler, DiaSimTime umbrella) are Not Started — see the Tasks tables below for the next entry points (2.1, 4.1/4.3). Three blocking open questions were resolved before implementation began; see "Spec Review Findings" below for the full design trail.
