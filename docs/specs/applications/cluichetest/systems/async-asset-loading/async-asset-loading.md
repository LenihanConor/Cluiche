...
|---------|-------------|------|--------|
| JobSystem Module | New `JobSystemModule` wrapping `JobSystem::Initialize/Shutdown`; manifest wiring on MainPU | [jobsystem-module.md](jobsystem-module.md) | Approved |
| TextureHandler Two-Phase Load | Refactor `TextureHandler::Load` into worker decode + main-thread upload; add `Tick()`; `AssetServiceModule::DoUpdate` calls it | [texturehandler-two-phase-load.md](texturehandler-two-phase-load.md) | Approved |
| AssetService Stage-Transition Watcher | `AssetServiceModule::DoUpdate` watches `Application::GetCurrentStage()` and auto-issues `RequestStageLoad` on transition; adds thread-safe `GetStatic()`, `IsStageLoadComplete`, `GetStageLoadState` accessors | [assetservice-stage-watcher.md](assetservice-stage-watcher.md) | Approved |
| DummyStage Asset Deployment | Ensure `Stages/DummyStage/World/Textures/test_*.png` exist on disk; create `dummy_stage.diaapp` if missing; remove DummyStage textures from global pre-load | [dummystage-asset-deployment.md](dummystage-asset-deployment.md) | Approved |
| DummyStage Async Load Consumer | `DummyLevelModule::DoStart` polls `AssetServiceModule::GetStatic()->IsStageLoadComplete("DummyStage")`; replaces placeholder `mStartLoadRequested`. No cross-PU method calls. | [dummystage-async-load-consumer.md](dummystage-async-load-consumer.md) | Approved |
...
| SD-013 | `TextureHandler::Tick()` produces `Dia::SFML::SfmlTexture` instances (concrete `ITexture` impls) registered in `mAssetIdToTexture`; callers retrieve textures via `LookupTexture(StringCRC) → ITexture*`. `unsigned int` runtime texture ids are removed. | Mandated by `texture-handle-stringcrc` feature (PD-001 / RB-007 compliance). `texturehandler-two-phase-load` plan must be re-targeted to produce `SfmlTexture` instances instead of `unsigned int` ids. | TextureHandler | Accepted | Yes |
