...
| 3 | Implement `MoveAgents`: per-frame sine-wave update + MarkDirty; destruction at frame 90 | `destroy_sweep` checkpoint passes | Done | sonnet | Depends on T2 |
| 4 | Implement `RunQueries` + `CheckCheckpoints`: all 5 query shapes, layer-mask reduction at frame 60, knearest k=5, rotating ray, all checkpoint booleans | All 8 checkpoints pass | Done | sonnet | Depends on T3 |
| 5 | Implement `UpdateMetrics`: emit 6 metrics per frame | Metric assertions pass | Done | haiku | Depends on T4 |
