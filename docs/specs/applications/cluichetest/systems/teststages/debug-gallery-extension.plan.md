...
|---|------|------|--------|-------|-------|
| 1 | Add forward declarations for 9 new fixture namespaces + 9 new domain namespaces to `DebugGalleryTestStageModule.h`; add 9 `std::unique_ptr<T>` fixture members + 9 `std::unique_ptr<T>` domain members inside `#ifdef DIA_DEBUG` | Build succeeds (header only change) | Done | haiku | Follow existing block layout — fixture members first, then domain members; Header: 6 fwd-decls, 8 fixture members, 6 domain members; kDescription→23 |
| 2 | Add 9 domain `#include` lines + 9 fixture `#include` lines in `.cpp`; construct fixtures in `OnStart` (`SteeringSystem` default; `PathGrid(8,8,1.0f)` + empty `PathResult`; `FlowField(8,8,1.0f)`; remaining 6 AI fixtures default-constructed); construct all 9 domain instances | Build succeeds; `OnStart` runs without assert | Done | sonnet | Pattern: fixture constructed first, domain constructed referencing it — mirror existing IK/Rig/Animation block; CPP: 6 domain includes + 7 fixture includes; fixtures+domains constructed in OnStart |
| 3 | Add 9 `vd->RegisterDomain(...)` calls in `OnUpdate`; update log message to `"21 domains registered"` | `dia run cluichetest` → DebugGallery → tilde key → 21 domain cards visible in panel | Done | haiku | Insert after `vd->RegisterDomain(*mMesh3DDomain)`; 6 RegisterDomain calls added; log updated to 21 domains registered |
| 4 | Add 9 `vd->UnregisterDomain(...)` + 9 `.reset()` calls in `OnStop` in correct order (domains before fixtures); update log to `"21 domains unregistered"` | Clean stop, no assert on re-enter | Done | haiku | Unregister domains in reverse registration order; reset domains before their fixtures; 6 UnregisterDomain + 6 domain reset + 8 fixture reset calls; log updated to 21 domains unregistered |
| 5 | Add 9 `<ProjectReference>` entries to `CluicheTest.vcxproj` + corresponding filter entries in `.vcxproj.filters` for all 9 new debugger vcxprojs | `msbuild` succeeds; no unresolved external | Done | haiku | Use `dia docs vcxproj-add` for each — or add all 9 in one manual edit following existing reference block; 6 ProjectReference entries added to CluicheTest.vcxproj |
| 6 | `dia run cluichetest` → navigate to `DebugGalleryTestStage` → press tilde → confirm 21 domain cards appear; tab through Navigation and AI/Behavior groups; confirm no crash after 300 frames | All 21 cards render; stage returns to Boot at frame 300 | Done | sonnet | Manual verification pass — quote panel card count from log output; Stage PASSED at frame 300; 21 domains registered/unregistered cleanly per log |
