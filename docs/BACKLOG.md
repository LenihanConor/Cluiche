...
| Editor `core_metrics` consumer migration | `GameConnectionController` and `gameconnection/index.html` still consume the proto `MESSAGE_TYPE_CORE_METRICS` topic for the live FPS/memory/frame-time panel. The hand-rolled broadcast block in `DebugServer::Tick` was deleted (2026-05-19); `ObservationBridge::OnSnapshot` now handles metric forwarding via `observation.metric` JSON topic. The editor panel needs porting to `observation.metric` to restore live data — belongs to DiaApplicationFlowEditor / live-data-visualization feature work. `ServerStats` struct stays: its fields are read by `DebugServerHostModule` to populate `MetricRegistry` gauges (`dia.debugserver.*`). |
