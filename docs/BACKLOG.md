...
| DiaMessageBus | All 9 features Approved ✅: module-and-build, core-bus, diagamemessages-format, schema-browser, entity-router-registration, eventdispatcher-removal, flush-adapters, frame-ledger, visual-debugger. Declaration-first (`.diagamemessages` → `dia codegen messages`); last-tick ledger + in-game visual debugger in scope; only Inspector tier deferred. Plan: [diamessagebus.plan.md](specs/applications/dia/systems/diamessagebus/diamessagebus.plan.md) — 12 tasks, 8 phases. Entry point: task 1 (module-and-build scaffold). 3 open Qs to resolve before their tasks (core-bus ledger cap, frame-ledger read shape/ownership). | DiaMailbox ✅, DiaStreams ✅, DiaApplicationFlow ✅, DiaObservation ✅ |
...
| MessageBusTestStage | [messagebus-test-stage.md](specs/applications/cluichetest/systems/teststages/messagebus-test-stage.md) | Approved ✅ — 5 Emitters + 5 Receivers; BroadcastRouter, EntityRouter (real diaentitytemplate entities), Reaction pass, IFlushAdapter all exercised; `MessageBusDebugDomain` live. Prereq: DiaMessageBus visual-debugger feature (ships with system). 7 tasks. |
