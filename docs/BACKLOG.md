...
| ~~DiaEditorAPI~~ | [diaeditorapi.md](specs/systems/cluicheeditor/diaeditorapi.md) ✅ | **All meaningful migrations complete.** Phase 1 done — action registry, Python `dia_editor` module, incremental regen, JSON capacity 256. All 6 scriptable plugins migrated: `asset-catalogue` ✅, `entity-template` ✅, `scene-editor` ✅, `plugin-browser` ✅, `app-editor-actions` ✅, `app-flow-editor` ✅. Inspector migrations dropped (observation-only). `pipeline-migration` Deferred. Phase 2 (MCP adapter) Deferred — needs DiaWebSocket. | DiaAPI, DiaEditor, DiaPython, DiaWebSocket (Ph2) |
