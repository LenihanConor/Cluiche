...
| DiaEntitySpawner | — | Needs `/spec-system` — runtime entity spawning from blueprints. `SpawnRequest` one-shot API + `SpawnEmitterComponent` (rate, burst, cap, lifetime/distance despawn). DiaObservation coverage. GoogleTest suite + CluicheTest E2E visual stage. Depends on diaentity ✅, DiaReflect ✅, DiaSerializer ✅. |
