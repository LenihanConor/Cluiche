...
|--------|------|----------|------------|
| DiaApplicationFlowEditor | [diaapplicationfloweditor.md](specs/systems/dia/diaapplicationfloweditor.md) | 17 features (all Approved + implemented). Tasks 1–55 Done. Task #56 (manual E2E launch test) is the only remaining gate. | DiaEditor ✅, DiaWebSocket ✅, DiaApplicationFlow ✅ |
...
| DiaApplicationFlow — v1 source removal | v1 types still live in `Dia/DiaApplicationFlow/`. **Progress:** v1 unit tests deleted, dead consumers removed, DiaDebugServer decoupled, 4 v1 integration tests deleted, `DiaApplicationEditor.cpp`/`ManifestSerializer`/`ManifestEditorData` replaced by v2 plugin (`DiaApplicationFlowEditorPlugin`). **Remaining consumer:** `Dia/DiaGame/GameFileComposer.{h,cpp}` — composes `.diagame` files via v1 `ApplicationManifest` + v1 `ManifestComposer`. Needs design decision: whether `GameFileComposer` becomes `ComposeFromGameFileV2` returning `ApplicationManifestV2` or the whole composition path moves to `ManifestComposerV2`. Not a cleanup — treat as its own feature with a spec. |
