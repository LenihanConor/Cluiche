...
| Scene2DTestStage | DiaScene2D | Scene loading, camera/light hydration, entity spawning, layer resolution | `scene.loaded`, `scene.cameras_hydrated`, `scene.lights_hydrated`, `scene.entities_spawned`, `scene.layers_resolved` | [Spec Approved](specs/features/cluichetest/teststages/scene2d-stage.md). Blocked on DiaScene2D implementation. |
