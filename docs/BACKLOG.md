...
| DiaVisualDebugger layer tag conflict | `dia check arch` flags `dia.softbody2dvisualdebugger` (`domain/physics/tools`) → `dia.debug.visualdebugger` (`domain/visual/tools`) as forbidden cross-domain, and the same pattern hits ~42 files across every domain visual debugger (RigidBody2D, Animation2D, Rig2D, IK2D, Mesh3D, Scene2D, Geometry2D, AICallout, FlowField, Pathfinding, EntitySpatial, Steering, GridVisibility, Lighting3D, Entity, AssetRuntime). Root cause: two specs disagree. `layer-formalisation` (Approved, task R6) planned to split `DiaVisualDebugger` into `DiaDebugDraw` (`foundation/services`) + renamed `DiaVisualDebugRenderer` (`domain/visual/tools`) so domain tools wouldn't touch the visual domain directly — that split was only stubbed (`Dia/DiaDebugDraw/DiaDebugDraw.vcxproj` compiles files straight out of `DiaVisualDebugger/` via relative path, duplicate-compiling them into both libs; never finished). `DiaDebugDomain` (later, **Done**) then built `IDebugDomain`/`DiaDebugDomainRegistry`/`DebugGroupAccents` directly inside `DiaVisualDebugger/Domain/`, and its binding decision SD-002 explicitly sanctions `DiaXxxVisualDebugger → DiaVisualDebugger` as the accepted dependency direction — contradicting R6. Complicating it further: `IVisualDebugger.h`/`IObjectRenderer.h`/etc. under `DiaVisualDebugger/` are themselves already compatibility shims forwarding to `DiaCore/DebugDraw/` (`foundation/core`), suggesting a third, more-recent migration is also mid-flight. Needs a proper audit of `DiaCore/DebugDraw/` vs `DiaVisualDebugger/` vs the abandoned `DiaDebugDraw` stub before deciding: retag `dia.debug.visualdebugger`'s `layer:` to `foundation/services` (blocked by its own real dependency on `dia.graphics`, tagged `domain/visual/core` — would create a new upward-dependency violation unless the DiaGraphics-touching pieces split out first), finish R6 for real, or reconcile with SD-002 some other way. |
