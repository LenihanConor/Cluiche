...
namespace Dia
{
	namespace DebugServer
	{
		...
		{
			if (!mActive.load(std::memory_order_acquire) || !mServer || mServer->GetConnectionCount() == 0)
				return;
		...
		void ObservationBridge::OnSnapshot(const Dia::Observation::Metric::MetricSnapshot& snapshot)
		{
			...
			for (unsigned int i = 0; i < snapshot.entryCount && pos < static_cast<int>(sizeof(buf)) - 256; ++i)
			{
