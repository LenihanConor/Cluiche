...
from dia_cli.commands import setup_cmd
from dia_cli.commands.setup import executable_cmd
from dia_cli.commands.setup import setpath_cmd
...
def cli(ctx):
    ...
    Should be run from the folder containing dia_cli_prime_config.json,
    which for standard projects is the project's root folder.

    If dia_cli_prime_config.json does not yet exist, this command will
...
def executable(ctx, name):
    ...
    """
    dia_cli_paths = DiaCLIPaths(Config.from_context(ctx))
    executable_cmd.customize_name(dia_cli_paths, name)
