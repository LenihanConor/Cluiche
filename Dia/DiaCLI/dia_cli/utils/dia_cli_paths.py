...
## @package dia_cli_paths Path-related utilities.
...
class DiaCLIPaths():
    ...
    def __init__(self, config, **kwargs):
        """!
        @param config a dia_cli_config.Config object
        """
        ## A dia_cli_config.Config object, to which this class delegates most of its responsibilities.
        self.config = config

    @staticmethod
    def from_mdk_context(dia_cli_context):
        """!Gets the DiaCLIPaths instance from a dia_cli_context, which is Click's ctx.obj.
        """
        return DiaCLIPaths(dia_cli_context)
    ...
    def external(self, subfolder='') -> (Path):
        ...
        which is `paths.external` in `dia_cli_prime_config.json`.
    ...
    def path(self, path_key) -> (Path):
        ...
        Prefer this way of getting paths over `dia_cli_config.Config.path()`.
    ...
    def prime_config(self) -> (Path):
        """!Get the path to dia_cli_prime_config.json"""
