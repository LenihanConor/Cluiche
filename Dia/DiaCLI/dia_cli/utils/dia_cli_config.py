...
## @package dia_cli_config Configuration-related utilities.

CONFIG_FILENAME_TEMPLATE = 'dia_cli_{profile}_config.json'
CONFIG_KEY = 'config'
...
class Config:
    ...
    Currently provides access to `dia_cli_prime_config.json` but will be extended to other configurations as they arise.
    ...
    DEFAULT_FEATURE_CONFIG_FILENAME = 'dia_cli_config.json'
    ...
    def save(self):
        """!Overwrites the contents of `dia_cli_prime_config.json` with the contents of this Config object."""
    ...
    def prime_config(self):
        """!Path to `dia_cli_prime_config.json`."""
    ...
    def path(self, path_key):
        ...
        Prefer `dia_cli_paths.DiaCLIPaths.path()` over this method to retrieve a specific path.
    ...
    def set_path(self, path_key, path):
        ...
        @param [in] path_key the path's key in the `paths` map in dia_cli_prime_config.json
    ...
    def has_path(self, path_key):
        """!Checks to see whether the path for `path_key` has been configured in `dia_cli_prime_config.json`
        @param [in] path_key the path's key in the `paths` map in `dia_cli_prime_config.json`
        """
