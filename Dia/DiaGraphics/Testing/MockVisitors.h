...
{
	// 7 entries: Circle2D=0 through Triangle2D=6
	mutable int primitiveCount[7] = {};  // indexed by DebugPrimitiveType
	mutable int textCount  = 0;
	...
	void VisitText(const DebugPrimitiveText2D& /*t*/) const override { ++textCount; }
	...
	int TextCount()     const { return textCount; }
...
struct InspectingDebugVisitor : public DebugFrameDataVisitor
{
	...
	mutable DebugPrimitiveText2D lastText    = {};
	mutable int                visitCount    = 0;
	mutable int                textVisitCount = 0;
	mutable int                frameCount    = 0;
	...
	void VisitText(const DebugPrimitiveText2D& t) const override
	{
		lastText = t;
		++textVisitCount;
	}

	void Reset() { visitCount = 0; textVisitCount = 0; frameCount = 0; }
};
