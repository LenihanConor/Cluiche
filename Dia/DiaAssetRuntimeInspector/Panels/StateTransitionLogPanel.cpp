	...
	namespace AssetRuntime
	{
		namespace Inspector
		{
				...
				SharedPluginState* state)
			{
				...
				if (mBridge)
				{
					mBridge->RegisterRequestHandler(
						Dia::Core::StringCRC("asset_runtime_inspector.log_pause"),
					...
					mBridge->RegisterRequestHandler(
						Dia::Core::StringCRC("asset_runtime_inspector.log_resume"),
						[this](const Json::Value& /*data*/) -> Json::Value
						{
							SetPaused(false);
							PushLogToUI();
					...
					mBridge->RegisterRequestHandler(
						Dia::Core::StringCRC("asset_runtime_inspector.log_clear"),
						[this](const Json::Value& /*data*/) -> Json::Value
						{
							ClearLog();
							Json::Value result;
					...
					mBridge->RegisterRequestHandler(
						Dia::Core::StringCRC("asset_runtime_inspector.log_set_max"),
						[this](const Json::Value& data) -> Json::Value
						{
							Json::Value result;
			...
			void StateTransitionLogPanel::Deactivate()
			{
				...
				if (mBridge)
				{
					mBridge->UnregisterRequestHandler(Dia::Core::StringCRC("asset_runtime_inspector.log_pause"));
					mBridge->UnregisterRequestHandler(Dia::Core::StringCRC("asset_runtime_inspector.log_resume"));
					mBridge->UnregisterRequestHandler(Dia::Core::StringCRC("asset_runtime_inspector.log_clear"));
					mBridge->UnregisterRequestHandler(Dia::Core::StringCRC("asset_runtime_inspector.log_set_max"));
				}
			...
			void StateTransitionLogPanel::OnConnectionStateChanged(bool connected)
			{
				...
				if (mBridge)
				{
					...
					mBridge->NotifyUIDataChanged("asset_runtime_inspector.log_connection_state", data);
				}
			...
			void StateTransitionLogPanel::PushLogToUI()
			{
				...
				mBridge->NotifyUIDataChanged("asset_runtime_inspector.log_data", data);
			}
			...
			void StateTransitionLogPanel::PushIncrementalToUI(const TransitionLogEntry& entry)
			{
				...
				data["total"] = static_cast<int>(mLogCount);
				data["paused"] = mPaused;
				mBridge->NotifyUIDataChanged("asset_runtime_inspector.log_entry", data);
			}
		}
	}
}
