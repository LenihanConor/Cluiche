	...
	namespace AssetRuntime
	{
		namespace Inspector
		{
				...
				SharedPluginState* state)
			{
				...
				if (mBridge)
				{
					mBridge->RegisterRequestHandler(
						Dia::Core::StringCRC("asset_runtime_inspector.force_refresh"),
					...
					mBridge->RegisterRequestHandler(
						Dia::Core::StringCRC("asset_runtime_inspector.set_poll_interval"),
						[this](const Json::Value& data) -> Json::Value
						{
							Json::Value result;
					...
					mBridge->RegisterRequestHandler(
						Dia::Core::StringCRC("asset_runtime_inspector.select_asset"),
						[this](const Json::Value& data) -> Json::Value
						{
							Json::Value result;
			...
			void AssetStateTablePanel::Deactivate()
			{
				...
				if (mBridge)
				{
					mBridge->UnregisterRequestHandler(Dia::Core::StringCRC("asset_runtime_inspector.force_refresh"));
					mBridge->UnregisterRequestHandler(Dia::Core::StringCRC("asset_runtime_inspector.set_poll_interval"));
					mBridge->UnregisterRequestHandler(Dia::Core::StringCRC("asset_runtime_inspector.select_asset"));
				}
			...
			void AssetStateTablePanel::OnConnectionStateChanged(bool connected)
			{
				...
				if (mBridge)
				{
					...
					mBridge->NotifyUIDataChanged("asset_runtime_inspector.table_connection_state", data);
				}
			...
			void AssetStateTablePanel::PushSnapshotToUI()
			{
				...
				mBridge->NotifyUIDataChanged("asset_runtime_inspector.snapshot", data);
			}
