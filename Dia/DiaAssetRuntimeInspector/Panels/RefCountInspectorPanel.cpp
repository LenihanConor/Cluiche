		...
		namespace Inspector
		{
			...
			void RefCountInspectorPanel::OnConnectionStateChanged(bool connected)
			{
				...
				if (mBridge)
				{
					...
					mBridge->NotifyUIDataChanged("asset_runtime_inspector.inspector_connection_state", data);
				}
			...
			void RefCountInspectorPanel::PushInspectorToUI()
			{
				...
				if (mLastInspectedAssetId == Dia::Core::StringCRC())
				{
					...
					mBridge->NotifyUIDataChanged("asset_runtime_inspector.inspector_data", data);
					return;
				}
