	...
	namespace AssetRuntime
	{
		namespace Inspector
		{
				...
				const AssetStateTablePanel* tablePanel)
			{
				...
				if (mBridge)
				{
					mBridge->RegisterRequestHandler(
						Dia::Core::StringCRC("asset_runtime_inspector.expand_stage"),
					...
					mBridge->RegisterRequestHandler(
						Dia::Core::StringCRC("asset_runtime_inspector.collapse_stage"),
						[this](const Json::Value& data) -> Json::Value
						{
							Json::Value result;
							if (data.isMember("stageId") && data["stageId"].isString())
					...
					mBridge->RegisterRequestHandler(
						Dia::Core::StringCRC("asset_runtime_inspector.tree_select_asset"),
						[this](const Json::Value& data) -> Json::Value
						{
							Json::Value result;
			...
			void StageAssetTreePanel::Deactivate()
			{
				...
				if (mBridge)
				{
					mBridge->UnregisterRequestHandler(Dia::Core::StringCRC("asset_runtime_inspector.expand_stage"));
					mBridge->UnregisterRequestHandler(Dia::Core::StringCRC("asset_runtime_inspector.collapse_stage"));
					mBridge->UnregisterRequestHandler(Dia::Core::StringCRC("asset_runtime_inspector.tree_select_asset"));
				}
			...
			void StageAssetTreePanel::OnConnectionStateChanged(bool connected)
			{
				...
				if (mBridge)
				{
					...
					mBridge->NotifyUIDataChanged("asset_runtime_inspector.tree_connection_state", data);
				}
			...
			void StageAssetTreePanel::PushTreeToUI()
			{
				...
				mBridge->NotifyUIDataChanged("asset_runtime_inspector.tree_data", data);
			}
				...
				bool success, const Json::Value& result)
			{
				...
				if (mBridge)
				{
					...
					mBridge->NotifyUIDataChanged("asset_runtime_inspector.stage_children", data);
				}
