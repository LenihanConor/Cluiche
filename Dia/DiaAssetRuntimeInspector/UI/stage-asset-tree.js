(function() {
    ...
    treeContainer.addEventListener('click', function(e) {
        ...
        if (stageNode) {
            ...
            if (stage && stage.expanded) {
                if (window.diaBridge)
                    window.diaBridge.sendRequest('asset_runtime_inspector.collapse_stage', { stageId: stageId });
            } else {
                if (window.diaBridge)
                    window.diaBridge.sendRequest('asset_runtime_inspector.expand_stage', { stageId: stageId });
            }
            return;
        }
        ...
        if (assetNode) {
            ...
            if (assetId && window.diaBridge) {
                window.diaBridge.sendRequest('asset_runtime_inspector.tree_select_asset', { assetId: assetId });
            }
        }
    });
    ...
    if (window.diaBridge) {
        window.diaBridge.onDataChanged('asset_runtime_inspector.tree_data', function(data) {
            treeData = data;
            renderTree();
        });

        window.diaBridge.onDataChanged('asset_runtime_inspector.stage_children', function(data) {
        ...
        window.diaBridge.onDataChanged('asset_runtime_inspector.tree_connection_state', function(data) {
            if (data.connected) {
