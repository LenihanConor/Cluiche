(function() {
    ...
    function reportFilterChange() {
        if (window.diaBridge) {
            window.diaBridge.sendRequest('asset_runtime_inspector.update_filters', {
    ...
    btnRefresh.addEventListener('click', function() {
        if (window.diaBridge) {
            window.diaBridge.sendRequest('asset_runtime_inspector.force_refresh', {});
        }
    });
    ...
    pollIntervalInput.addEventListener('change', function() {
        ...
        if (!isNaN(val) && val >= 0.1 && window.diaBridge) {
            window.diaBridge.sendRequest('asset_runtime_inspector.set_poll_interval', { interval: val });
        }
    });
    ...
    tableBody.addEventListener('click', function(e) {
        ...
        if (window.diaBridge) {
            window.diaBridge.sendRequest('asset_runtime_inspector.select_asset', { assetId: id });
        }
    });
    ...
    if (window.diaBridge) {
        window.diaBridge.onDataChanged('asset_runtime_inspector.snapshot', function(data) {
            allAssets = data.assets || [];
            statusTime.textContent = 'Last refresh: ' + new Date().toLocaleTimeString();
            applyFilters();
        });

        window.diaBridge.onDataChanged('asset_runtime_inspector.table_connection_state', function(data) {
            if (data.connected) {
        ...
        window.diaBridge.onDataChanged('asset_runtime_inspector.table_filters', function(data) {
