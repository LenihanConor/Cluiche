(function() {
    ...
    btnPause.addEventListener('click', function() {
        if (paused) {
            if (window.diaBridge) window.diaBridge.sendRequest('asset_runtime_inspector.log_resume', {});
            paused = false;
            btnPause.textContent = 'Pause';
            btnPause.classList.remove('active');
        } else {
            if (window.diaBridge) window.diaBridge.sendRequest('asset_runtime_inspector.log_pause', {});
            paused = true;
    ...
    btnClear.addEventListener('click', function() {
        if (window.diaBridge) window.diaBridge.sendRequest('asset_runtime_inspector.log_clear', {});
    });
    ...
    maxEntriesInput.addEventListener('change', function() {
        ...
        if (!isNaN(val) && val >= 10 && window.diaBridge) {
            window.diaBridge.sendRequest('asset_runtime_inspector.log_set_max', { max: val });
        }
    });

    // Bridge
    if (window.diaBridge) {
        window.diaBridge.onDataChanged('asset_runtime_inspector.log_data', function(data) {
        ...
        window.diaBridge.onDataChanged('asset_runtime_inspector.log_connection_state', function(data) {
            if (data.connected) {
