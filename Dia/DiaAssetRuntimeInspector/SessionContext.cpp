...
namespace Dia
{
	namespace AssetRuntime
	{
		namespace Inspector
		{
			static const char* kContextFileName = "DiaAssetRuntimeInspector.context.json";
			...
			void SessionContext::Load(const char* outputDir)
			{
				...
				{
					DIA_LOG_WARNING("Editor", "DiaAssetRuntimeInspector SessionContext: failed to parse context JSON");
					return;
				}
			...
			void SessionContext::Save(const char* outputDir) const
			{
				...
				if (!file.is_open())
				{
					DIA_LOG_WARNING("Editor", "DiaAssetRuntimeInspector SessionContext: could not write context file");
					return;
				}
