...
void Coord2DBoundsDrawer::Draw(Dia::Core::IDebugDraw& draw)
{
    ...
    draw.RequestDrawText(inBL, buf, 11.0f, Dia::Debug::DebugColourPalette::kActive);

    // Top-right
    snprintf(buf, sizeof(buf), "(%.1f, %.1f)", tr.x, tr.y);
    draw.RequestDrawText(inTR, buf, 11.0f, Dia::Debug::DebugColourPalette::kActive);

    // Top-left
    snprintf(buf, sizeof(buf), "(%.1f, %.1f)", bl.x, tr.y);
    draw.RequestDrawText(inTL, buf, 11.0f, Dia::Debug::DebugColourPalette::kActive);

    // Bottom-right
    snprintf(buf, sizeof(buf), "(%.1f, %.1f)", tr.x, bl.y);
    draw.RequestDrawText(inBR, buf, 11.0f, Dia::Debug::DebugColourPalette::kActive);
}
