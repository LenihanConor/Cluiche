...
namespace Dia
{
	...
	{
		static const Dia::Core::StringCRC kEntityTypeId("diaentitytemplate");
		...
		{
			DIA_TRACE_ZONE("entity_template_editor.build_list", Dia::Observation::Trace::Category::kNone);
			Json::Value result;
			result["success"] = true;

			Json::Value groups(Json::arrayValue);
			if (entityRecords.isArray() && entityRecords.size() > 0)
				groups.append(BuildGroup("Entity", "diaentitytemplate", entityRecords));
