...
void SpringConstraint::PreStep(float dt)
{
    mDt = dt;
    ...
    // Soft constraint: compliance = 1/stiffness
    // Effective mass: 1 / (invMass_structural + compliance / dt^2)
    ...
    const float kStructural = invMA + invMB + raCrossN * raCrossN * invIA + rbCrossN * rbCrossN * invIB;

    const float compliance = (mStiffness > 1e-10f) ? 1.0f / mStiffness : 0.0f;
    const float kSoft = kStructural + compliance / (dt * dt);
    mEffMass = (kSoft > 1e-10f) ? 1.0f / kSoft : 0.0f;

    const float C = dist - mRestLength;
    mBias = mStiffness * C * mEffMass;
...
void SpringConstraint::ApplyImpulse()
{
    ...
    // Spring + damping impulse: -k*(dist - rest) * dt - d * relVel
    // Simplified: lambda from velocity Jacobian plus spring force applied as an impulse
    const float springForce = mStiffness > 1e-10f
        ? -(relVelAlongAxis + mBias) * mEffMass
        : 0.0f;

    const float lambda = springForce - mDamping * relVelAlongAxis * mEffMass;
...
bool SpringConstraint::InvolvesBody(const RigidBody2D* body) const
{
