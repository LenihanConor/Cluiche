...
def test_softbody2d_rope_settles(dia_client):
    ...
    result = dia_client.poll_checkpoint(_ROPE_CHECKPOINT, timeout_s=14.0)
...
def test_softbody2d_cloth_settles(dia_client):
    ...
    result = dia_client.poll_checkpoint(_CLOTH_CHECKPOINT, timeout_s=14.0)
...
def test_softbody2d_rope_settles_before_cloth(dia_client, assert_metric):
    ...
    rope_result  = dia_client.poll_checkpoint(_ROPE_CHECKPOINT,  timeout_s=14.0)
    cloth_result = dia_client.poll_checkpoint(_CLOTH_CHECKPOINT, timeout_s=14.0)
...
def test_softbody2d_determinism(dia_client):
    ...
    dia_client.navigate_to("SoftBody2DTestStage")
    r1_rope  = dia_client.poll_checkpoint(_ROPE_CHECKPOINT,  timeout_s=14.0)
    r1_cloth = dia_client.poll_checkpoint(_CLOTH_CHECKPOINT, timeout_s=14.0)
    assert r1_rope["passed"],  f"Run 1 rope failed: {r1_rope['message']}"
    assert r1_cloth["passed"], f"Run 1 cloth failed: {r1_cloth['message']}"
    dia_client.navigate_to("Boot")

    dia_client.navigate_to("SoftBody2DTestStage")
    r2_rope  = dia_client.poll_checkpoint(_ROPE_CHECKPOINT,  timeout_s=14.0)
    r2_cloth = dia_client.poll_checkpoint(_CLOTH_CHECKPOINT, timeout_s=14.0)
