...
{
	EXPECT_STREQ(BlueprintFileHandler::TopLevelKeyForExtension(".diaentity"), "entity_blueprint");
}
...
TEST(BlueprintFileHandler, Load_NonexistentFile_ReturnsFalse)
{
	...
	Json::Value root;
	char err[128] = {};
	EXPECT_FALSE(handler.Load("nonexistent_blueprint_xyz.diaentity", root, err, sizeof(err)));
	EXPECT_GT(strlen(err), 0u);
}
...
{
	const char* kPath = "tmp_test_blueprint_load.diaentity";
...
{
	const char* kPath = "tmp_test_blueprint_malformed.diaentity";
...
{
	const char* kPathA = "tmp_roundtrip_a.diaentity";
	const char* kPathB = "tmp_roundtrip_b.diaentity";
	const char* kJson = R"({
	"entity_blueprint": {
		"id": "player",
		"components": [
