...
{
    // At 90-degree CW rotation world point (10, 0) should shift along the
    // screen Y-axis rather than the screen X-axis.
    // With Y-down screen convention a CW rotation maps world +X → screen +Y,
    // so the screen offset from centre should be (0, zoom * 10).
    Camera2D cam;
    cam.SetRotation(90.0f);
    ViewportTransform vt(cam, kWindow);
    ...
    // The displacement must be purely along Y (no X component).
    EXPECT_NEAR(dx, 0.0f, kTol);
    // The Y displacement magnitude must equal 10 (zoom=1).
    EXPECT_NEAR(std::abs(dy), 10.0f, kTol);
}

TEST(ViewportTransform_Rotation, RoundTrip_45Degrees)
{
    // ScreenToWorld(WorldToScreen(p)) must recover p at 45-degree rotation.
    Camera2D cam;
...
{
    // Very small (but non-zero) zoom should not produce infinities or NaNs.
    Camera2D cam;
    cam.SetZoom(0.001f);
    ViewportTransform vt(cam, kWindow);
...
{
    // WorldToScreen with very large values must not crash and must be finite.
    Camera2D cam;
    ViewportTransform vt(cam, kWindow);
...
{
    // WorldToScreen with negative coordinates must return a valid screen point.
    Camera2D cam;
    ViewportTransform vt(cam, kWindow);
...
{
    // A 1x1 window should not crash; world bounds should be near-zero in size.
    Camera2D cam;
...
{
    // A default-constructed FrameData must expose a Camera2D at origin,
    // zoom 1, rotation 0 — matching the Camera2D default.
