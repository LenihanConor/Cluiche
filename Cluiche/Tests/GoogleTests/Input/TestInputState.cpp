...
TEST(InputState, KeyPressedAndReleased)
{
	...
	pressEvent.key.code = static_cast<int>(EKey::W); pressEvent.key.alt = false; pressEvent.key.control = false; pressEvent.key.shift = false; pressEvent.key.system = false;
	...
	releaseEvent.key.code = static_cast<int>(EKey::W); releaseEvent.key.alt = false; releaseEvent.key.control = false; releaseEvent.key.shift = false; releaseEvent.key.system = false;
...
TEST(InputState, MultipleKeysDown)
{
	...
	wPress.key.code = static_cast<int>(EKey::W);
	wPress.key.alt = false;
	wPress.key.control = false;
	wPress.key.shift = false;
	wPress.key.system = false;
	events.Add(wPress);

	Event aPress;
	aPress.type = Event::EType::kKeyPressed;
	aPress.key.code = static_cast<int>(EKey::A);
	aPress.key.alt = false;
	aPress.key.control = false;
	aPress.key.shift = false;
	aPress.key.system = false;
	events.Add(aPress);

	Event spacePress;
	spacePress.type = Event::EType::kKeyPressed;
	spacePress.key.code = static_cast<int>(EKey::Space);
	spacePress.key.alt = false;
	spacePress.key.control = false;
	spacePress.key.shift = false;
	spacePress.key.system = false;
...
TEST(InputState, MouseButtonPressedAndReleased)
{
	...
	pressEvent.mouseButton.button = static_cast<int>(EMouseButton::kLeft); pressEvent.mouseButton.x = 0; pressEvent.mouseButton.y = 0;
	events.Add(pressEvent);

	state.ProcessEvents(events);

	EXPECT_TRUE(state.IsMouseButtonDown(EMouseButton::kLeft));
	EXPECT_FALSE(state.IsMouseButtonDown(EMouseButton::kRight));
	...
	releaseEvent.mouseButton.button = static_cast<int>(EMouseButton::kLeft); releaseEvent.mouseButton.x = 0; releaseEvent.mouseButton.y = 0;
	events.Add(releaseEvent);

	state.ProcessEvents(events);

	EXPECT_FALSE(state.IsMouseButtonDown(EMouseButton::kLeft));
}
...
TEST(InputState, GamepadConnectedAndDisconnected)
{
	...
	connectEvent.consoleGamepadConnectEvent.gamepadIndex = 0;
	...
	disconnectEvent.consoleGamepadConnectEvent.gamepadIndex = 0;
...
TEST(InputState, GamepadAnalogStick)
{
	...
	// Move left stick
	Event stickEvent;
	stickEvent.type = Event::EType::kConsoleGamepadAnalogStickMove;
	stickEvent.consoleGamepadMoveEvent.gamepadIndex = 0;
	stickEvent.consoleGamepadMoveEvent.button = static_cast<int>(ConsoleGamepad::EButtonID::L_Thumbstick);
	stickEvent.consoleGamepadMoveEvent.x = 0.5f;
	stickEvent.consoleGamepadMoveEvent.y = -0.75f;
	events.Add(stickEvent);

	state.ProcessEvents(events);

	float x, y;
	state.GetGamepadAnalogStick(0, ConsoleGamepad::EButtonID::L_Thumbstick, x, y);
...
TEST(InputState, MultipleInputDevicesSimultaneously)
{
	...
	keyEvent.key.code = static_cast<int>(EKey::Space); keyEvent.key.alt = false; keyEvent.key.control = false; keyEvent.key.shift = false; keyEvent.key.system = false;
	events.Add(keyEvent);
