...
TEST(KeyboardEvents, DispatchKeyPressedEvent)
{
	...
	KeyPressedEvent* evt = new KeyPressedEvent(EKey::Return, false, false, false, false);
	dispatcher.Dispatch(evt);

	EXPECT_TRUE(received);
	EXPECT_EQ(receivedKey, EKey::Return);
}
...
{
	MouseButtonPressedEvent evt(EMouseButton::kLeft, 150, 250);

	EXPECT_EQ(evt.GetEventType(), MouseButtonPressedEvent::GetStaticType());
	EXPECT_EQ(evt.GetButton(), EMouseButton::kLeft);
...
{
	MouseButtonReleasedEvent evt(EMouseButton::kRight, 300, 400);

	EXPECT_EQ(evt.GetEventType(), MouseButtonReleasedEvent::GetStaticType());
	EXPECT_EQ(evt.GetButton(), EMouseButton::kRight);
...
TEST(MouseEvents, DispatchMouseEvents)
{
	...
	dispatcher.Dispatch(new MouseButtonPressedEvent(EMouseButton::kLeft, 150, 150));
...
{
	GamepadAnalogStickMoveEvent evt(0, ConsoleGamepad::EButtonID::L_Thumbstick, 0.5f, -0.75f);

	EXPECT_EQ(evt.GetEventType(), GamepadAnalogStickMoveEvent::GetStaticType());
	EXPECT_EQ(evt.GetGamepadIndex(), 0u);
	EXPECT_EQ(evt.GetStick(), ConsoleGamepad::EButtonID::L_Thumbstick);
...
{
	GamepadTriggerEvent evt(0, ConsoleGamepad::EButtonID::L_Trigger, 0.8f);

	EXPECT_EQ(evt.GetEventType(), GamepadTriggerEvent::GetStaticType());
	EXPECT_EQ(evt.GetGamepadIndex(), 0u);
	EXPECT_EQ(evt.GetTrigger(), ConsoleGamepad::EButtonID::L_Trigger);
...
TEST(GamepadEvents, DispatchGamepadEvents)
{
	...
	dispatcher.Dispatch(new GamepadAnalogStickMoveEvent(0, ConsoleGamepad::EButtonID::L_Thumbstick, 1.0f, 0.0f));
...
// TODO: EventDispatcher doesn't support priority yet
...
TEST(ModernEvents, DispatchMixedInputTypes)
{
	...
	dispatcher.Dispatch(new MouseButtonPressedEvent(EMouseButton::kLeft, 100, 100));
	dispatcher.Dispatch(new GamepadButtonPressedEvent(0, ConsoleGamepad::EButtonID::A));
	dispatcher.Dispatch(new JoystickButtonPressedEvent(0, 0));
...
TEST(ModernEvents, EventDataPreservedThroughDispatch)
{
	...
	EKey receivedKey = EKey::Unknown;
	EMouseButton receivedButton = EMouseButton::kLeft;
	unsigned int receivedGamepadIndex = 999;
	ConsoleGamepad::EButtonID receivedGamepadButton = ConsoleGamepad::EButtonID::A;
	...
	dispatcher.Dispatch(new KeyPressedEvent(EKey::Return, false, false, false, false));
	dispatcher.Dispatch(new MouseButtonPressedEvent(EMouseButton::kMiddle, 50, 50));
	dispatcher.Dispatch(new GamepadButtonPressedEvent(2, ConsoleGamepad::EButtonID::X));

	EXPECT_EQ(receivedKey, EKey::Return);
	EXPECT_EQ(receivedButton, EMouseButton::kMiddle);
