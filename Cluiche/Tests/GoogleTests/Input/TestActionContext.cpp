...
TEST(ActionContextManager, TopContextProcessedFirst)
{
	...
	menu->BindKey(actionInMenu, EKey::Return);
	...
	spaceEvt.key.code = static_cast<int>(EKey::Space);
	spaceEvt.key.alt = false;
	spaceEvt.key.control = false;
	spaceEvt.key.shift = false;
	spaceEvt.key.system = false;
	events.Add(spaceEvt);

	Event enterEvt;
	enterEvt.type = Event::EType::kKeyPressed;
	enterEvt.key.code = static_cast<int>(EKey::Return);
	enterEvt.key.alt = false;
	enterEvt.key.control = false;
	enterEvt.key.shift = false;
	enterEvt.key.system = false;
	events.Add(enterEvt);
...
TEST(ActionContextManager, BlockingContextPreventsLowerContexts)
{
	...
	ActionID jumpAction("Jump");
	gameplay->BindKey(jumpAction, EKey::Space);
	menu->BindKey(ActionID("MenuAction"), EKey::Return);
...
TEST(ActionContextManager, BlockingAppliesFromTopToBottom)
{
	...
	dialog->BindKey(dialogAction, EKey::Return);
	...
	wEvt.key.code = static_cast<int>(EKey::W);
	wEvt.key.alt = false;
	wEvt.key.control = false;
	wEvt.key.shift = false;
	wEvt.key.system = false;
	events.Add(wEvt);

	Event iEvt;
	iEvt.type = Event::EType::kKeyPressed;
	iEvt.key.code = static_cast<int>(EKey::I);
	iEvt.key.alt = false;
	iEvt.key.control = false;
	iEvt.key.shift = false;
	iEvt.key.system = false;
	events.Add(iEvt);
...
TEST(ActionContextManager, GameplayToMenuTransition)
{
	...
	menu->BindKey(selectAction, EKey::Return);
	...
	escapeEvt.key.code = static_cast<int>(EKey::Escape);
	escapeEvt.key.alt = false;
	escapeEvt.key.control = false;
	escapeEvt.key.shift = false;
	escapeEvt.key.system = false;
