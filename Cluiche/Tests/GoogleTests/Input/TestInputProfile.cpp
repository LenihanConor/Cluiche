...
TEST_F(InputProfileTest, SaveAndLoadMultipleBindings)
{
	...
	mouseEvt.mouseButton.x = 0;
	mouseEvt.mouseButton.y = 0;
	events.Add(mouseEvt);
...
TEST_F(InputProfileTest, SaveAndLoadAllBindingTypes)
{
	...
	actionMap.BindMouseButton(ActionID("MouseAction"), EMouseButton::kRight);
	...
	Event mouseEvt;
	mouseEvt.type = Event::EType::kMouseButtonPressed;
	mouseEvt.mouseButton.button = static_cast<int>(EMouseButton::kRight);
	mouseEvt.mouseButton.x = 0;
	mouseEvt.mouseButton.y = 0;
...
TEST_F(InputProfileTest, ActionMapLoadProfileConvenienceMethod)
{
	...
	ActionMap actionMap;
	actionMap.BindMouseButton(ActionID("Fire"), EMouseButton::kLeft);
...
TEST_F(InputProfileTest, LoadProfileClearsExistingBindings)
{
	...
	aEvt.key.code = static_cast<int>(EKey::A);
	aEvt.key.alt = false;
	aEvt.key.control = false;
	aEvt.key.shift = false;
	aEvt.key.system = false;
	events1.Add(aEvt);
	...
	bEvt.key.code = static_cast<int>(EKey::B);
	bEvt.key.alt = false;
	bEvt.key.control = false;
	bEvt.key.shift = false;
	bEvt.key.system = false;
	events2.Add(bEvt);
