...
describe("EditorBridge – sendEvent helpers", () => {
    it("shellReady calls dia.callCpp with shell_ready type", async () => {
        ...
        expect(window.dia.callCpp).toHaveBeenCalledWith("DiaEditor_call", expect.stringContaining('"type":"shell_ready"'));
    });
    it("undo calls dia.callCpp with undo type", async () => {
        const { EditorBridge } = await import("./EditorBridge");
        EditorBridge.undo();
        expect(window.dia.callCpp).toHaveBeenCalledWith("DiaEditor_call", expect.stringContaining('"type":"undo"'));
    });
    it("redo calls dia.callCpp with redo type", async () => {
        const { EditorBridge } = await import("./EditorBridge");
        EditorBridge.redo();
        expect(window.dia.callCpp).toHaveBeenCalledWith("DiaEditor_call", expect.stringContaining('"type":"redo"'));
    });
    it("togglePanelVisibility sends correct panel name", async () => {
        const { EditorBridge } = await import("./EditorBridge");
        EditorBridge.togglePanelVisibility("OutputConsole");
        const payload = JSON.parse(window.dia.callCpp.mock.calls[0][1]);
    ...
    it("drops event with warning when dia.callCpp is unavailable", async () => {
        ...
        const { EditorBridge } = await import("./EditorBridge");
        EditorBridge.shellReady();
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("dia.callCpp not available"), expect.anything());
    });
});
describe("EditorBridge – sendRequest / response dispatch", () => {
    ...
    it("resolves the correct promise when DiaEditor_onResponse fires", async () => {
        ...
        const reqId = JSON.parse(window.dia.callCpp.mock.calls[0][1]).reqId;
...
describe("EditorBridge – subscribe / topic listeners", () => {
    ...
    it("listener error is swallowed and other listeners still fire", async () => {
        ...
        expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("topic listener failed"), "err_topic", expect.any(Error));
    });
    it("broadcasts data-changed events to all iframes", async () => {
        ...
        expect(postSpy).toHaveBeenCalledWith({ __dia: true, topic: "broadcast_topic", data: { x: 1 } }, "*");
...
describe("EditorBridge – iframe relay", () => {
    it("routes a frame request through dia.callCpp", async () => {
        ...
        expect(window.dia.callCpp).toHaveBeenCalledWith("DiaEditor_call", expect.stringContaining('"type":"get_panels"'));
    });
    ...
    it("routes response back to the iframe window that sent the request", async () => {
        ...
        expect(postSpy).toHaveBeenCalledWith(expect.objectContaining({ __diaResponse: true, reqId: "frame-r42" }), "*");
    });
    it("ignores messages without __diaFromFrame flag", async () => {
        await import("./EditorBridge");
        const callSpy = window.dia.callCpp;
        callSpy.mockClear();
        window.dispatchEvent(new MessageEvent("message", {
